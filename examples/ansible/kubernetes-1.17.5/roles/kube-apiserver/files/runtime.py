...
COMPONENT='kube-apiserver';PATHS=['/etc/kubernetes/manifests/kube-apiserver.yaml'];SERVICES=[]
def precheck():
    for f in ['apiserver.crt','apiserver.key','apiserver-etcd-client.crt','apiserver-etcd-client.key','sa.pub']:assert (PKI/f).is_file(),f
    assert exists('/etc/kubernetes/encryption-config.yaml')
    assert containers('etcd'),'etcd must be healthy before API server installation'
def install():
    args=['kube-apiserver','--advertise-address='+IP,'--bind-address=0.0.0.0','--secure-port=6443','--insecure-port=0','--allow-privileged=true','--authorization-mode=Node,RBAC','--enable-admission-plugins=NodeRestriction','--enable-bootstrap-token-auth=true','--service-cluster-ip-range='+I['service_cidr'],'--service-account-key-file='+str(PKI/'sa.pub'),'--client-ca-file='+str(PKI/'ca.crt'),'--tls-cert-file='+str(PKI/'apiserver.crt'),'--tls-private-key-file='+str(PKI/'apiserver.key'),'--kubelet-client-certificate='+str(PKI/'apiserver-kubelet-client.crt'),'--kubelet-client-key='+str(PKI/'apiserver-kubelet-client.key'),'--kubelet-preferred-address-types=InternalIP,Hostname','--etcd-servers='+','.join('https://'+HOSTS[m]+':2379' for m in MASTERS),'--etcd-cafile='+str(PKI/'etcd/ca.crt'),'--etcd-certfile='+str(PKI/'apiserver-etcd-client.crt'),'--etcd-keyfile='+str(PKI/'apiserver-etcd-client.key'),'--encryption-provider-config=/etc/kubernetes/encryption-config.yaml','--requestheader-client-ca-file='+str(PKI/'front-proxy-ca.crt'),'--requestheader-allowed-names=front-proxy-client','--requestheader-extra-headers-prefix=X-Remote-Extra-','--requestheader-group-headers=X-Remote-Group','--requestheader-username-headers=X-Remote-User','--proxy-client-cert-file='+str(PKI/'front-proxy-client.crt'),'--proxy-client-key-file='+str(PKI/'front-proxy-client.key')]
    write(PATHS[0],static_pod('kube-apiserver',args,[(str(PKI),True),('/etc/kubernetes/encryption-config.yaml',True)]))
def postcheck():
    wait_for(lambda:kubectl(['--server=https://'+IP+':6443','get','--raw=/healthz'])=='ok')
    assert json.loads(kubectl(['--server=https://'+IP+':6443','version','-o','json']))['serverVersion']['gitVersion']=='v1.17.5'
    manifest=json.loads(pathlib.Path(PATHS[0]).read_text());assert '--encryption-provider-config=/etc/kubernetes/encryption-config.yaml' in manifest['spec']['containers'][0]['command']
def rollback_precheck():
    for name in ['kube-controller-manager','kube-scheduler']:assert not exists('/etc/kubernetes/manifests/'+name+'.yaml'),'Dependent control-plane component remains'
def rollback():remove(PATHS[0]);wait_for(lambda:not containers('kube-apiserver'))
def rollback_postcheck():assert not containers('kube-apiserver')
