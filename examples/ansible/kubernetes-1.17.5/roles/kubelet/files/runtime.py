...
COMPONENT='kubelet';PATHS=['/var/lib/kubelet','/etc/systemd/system/kubelet.service.d/20-clusterforge.conf', '/etc/kubernetes/manifests'];SERVICES=['kubelet']
def precheck():
    manifest_dir=pathlib.Path('/etc/kubernetes/manifests')
    if not MARKER.exists():assert not manifest_dir.exists() or (manifest_dir.is_dir() and not manifest_dir.is_symlink() and not any(manifest_dir.iterdir())), 'Unowned static Pod directory contents'
    assert I['kubelet_data_dir']=='/var/lib/kubelet'
    assert exists('/etc/kubernetes/kubelet.conf')
    assert run(['docker','info','--format','{{.CgroupDriver}}'])=='cgroupfs'
def install():
    write('/var/lib/kubelet/config.yaml',{'apiVersion':'kubelet.config.k8s.io/v1beta1','kind':'KubeletConfiguration','address':'0.0.0.0','staticPodPath':'/etc/kubernetes/manifests','clusterDNS':[I['cluster_dns_ip']],'clusterDomain':'cluster.local','cgroupDriver':'cgroupfs','authentication':{'anonymous':{'enabled':False},'webhook':{'enabled':True},'x509':{'clientCAFile':str(PKI/'ca.crt')}},'authorization':{'mode':'Webhook'},'rotateCertificates':True,'resolvConf':'/run/systemd/resolve/resolv.conf','failSwapOn':True})
    write(PATHS[1],'[Service]\nExecStart=\nExecStart=/usr/bin/kubelet --config=/var/lib/kubelet/config.yaml --kubeconfig=/etc/kubernetes/kubelet.conf --container-runtime=docker --network-plugin=cni --pod-infra-container-image='+image('pause')+' --node-ip='+IP+' --hostname-override='+NODE+' --root-dir=/var/lib/kubelet\n',0o644)
    pathlib.Path('/etc/kubernetes/manifests').mkdir(parents=True,exist_ok=True)
    run(['systemctl','daemon-reload']);run(['systemctl','enable','kubelet']);run(['systemctl','restart','kubelet'])
def postcheck():
    import urllib.request
    assert service_state('kubelet')['active']=='active'
    def healthy():
        with urllib.request.urlopen('http://127.0.0.1:10248/healthz',timeout=3) as r:return r.read()==b'ok'
    wait_for(healthy)
    assert json.loads(pathlib.Path('/var/lib/kubelet/config.yaml').read_text())['cgroupDriver']=='cgroupfs'
def rollback_precheck():
    manifest_dir=pathlib.Path('/etc/kubernetes/manifests')
    assert not manifest_dir.is_symlink(),'Unowned static Pod directory link'
    assert not manifest_dir.exists() or not any(manifest_dir.iterdir()),'Remove all owned static Pods before kubelet; unknown remaining content blocks rollback'
def rollback():
    run(['systemctl','stop','kubelet'])
    ids=run(['docker','ps','-aq','--filter','label=io.kubernetes.pod.namespace']).split()
    for cid in ids:run(['docker','rm','-f',cid])
    assert not mounted_under('/var/lib/kubelet'),'Kubelet mounts remain'
def rollback_postcheck():
    assert not run(['docker','ps','-q','--filter','label=io.kubernetes.pod.namespace'])
