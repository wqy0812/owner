...
COMPONENT='kube-proxy';PATHS=[];SERVICES=[]
def capture_extra():return {'network':network_snapshot()}
def restore_extra(s):network_restore(s['network'],['KUBE-'])
def verify_extra(s):network_restored(s['network'])
def objects():
    ns='kube-system';name='clusterforge-kube-proxy'
    cfg={'apiVersion':'kubeproxy.config.k8s.io/v1alpha1','kind':'KubeProxyConfiguration','bindAddress':'0.0.0.0','clientConnection':{'kubeconfig':'/var/lib/kube-proxy/kubeconfig.conf'},'clusterCIDR':I['pod_network_cidr'],'mode':'iptables'}
    kc={'apiVersion':'v1','kind':'Config','clusters':[{'name':'default','cluster':{'server':'https://'+I['control_plane_endpoint'],'certificate-authority':'/var/run/secrets/kubernetes.io/serviceaccount/ca.crt'}}],'users':[{'name':'default','user':{'tokenFile':'/var/run/secrets/kubernetes.io/serviceaccount/token'}}],'contexts':[{'name':'default','context':{'cluster':'default','user':'default'}}],'current-context':'default'}
    pod={'serviceAccountName':name,'hostNetwork':True,'priorityClassName':'system-node-critical','tolerations':[{'operator':'Exists'}],'containers':[{'name':'kube-proxy','image':image('kube_proxy'),'command':['/usr/local/bin/kube-proxy','--config=/var/lib/kube-proxy/config.conf','--hostname-override=$(NODE_NAME)'],'env':[{'name':'NODE_NAME','valueFrom':{'fieldRef':{'fieldPath':'spec.nodeName'}}}],'securityContext':{'privileged':True},'volumeMounts':[{'name':'config','mountPath':'/var/lib/kube-proxy'},{'name':'modules','mountPath':'/lib/modules','readOnly':True},{'name':'xtables','mountPath':'/run/xtables.lock'}]}],'volumes':[{'name':'config','configMap':{'name':name}},{'name':'modules','hostPath':{'path':'/lib/modules'}},{'name':'xtables','hostPath':{'path':'/run/xtables.lock','type':'FileOrCreate'}}]}
    return [{'apiVersion':'v1','kind':'ServiceAccount','metadata':{'name':name,'namespace':ns}}, {'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRoleBinding','metadata':{'name':name},'roleRef':{'kind':'ClusterRole','name':'system:node-proxier','apiGroup':'rbac.authorization.k8s.io'},'subjects':[{'kind':'ServiceAccount','name':name,'namespace':ns}]},{'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':name,'namespace':ns},'data':{'config.conf':json.dumps(cfg),'kubeconfig.conf':json.dumps(kc)}},{'apiVersion':'apps/v1','kind':'DaemonSet','metadata':{'name':name,'namespace':ns},'spec':{'selector':{'matchLabels':{'app':name}},'template':{'metadata':{'labels':{'app':name}},'spec':pod}}}]
def precheck():
    assert service_state('kubelet')['active']=='active'
    if NODE==PRIMARY:assert kubectl(['get','--raw=/healthz'])=='ok'
def install():
    if NODE==PRIMARY:apply_objects(objects())
def postcheck():
    wait_for(lambda:check_container('kube-proxy'))
    if NODE==PRIMARY:
        def ready():
            d=get_object('daemonset','clusterforge-kube-proxy','kube-system');return d and d.get('status',{}).get('numberReady')==6
        wait_for(ready)
def rollback():
    if NODE==PRIMARY:delete_objects()
    wait_for(lambda:not containers('kube-proxy'))
    cleanup_netfilter(['KUBE-'])
def rollback_postcheck():
    assert not containers('kube-proxy')
