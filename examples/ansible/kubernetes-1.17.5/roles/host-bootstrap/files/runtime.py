...
COMPONENT='host-bootstrap';PATHS=['/etc/sysctl.d/90-clusterforge-k8s1175.conf','/etc/modules-load.d/clusterforge-k8s1175.conf'];SERVICES=[]
KEYS=['net.ipv4.ip_forward','net.bridge.bridge-nf-call-iptables','net.bridge.bridge-nf-call-ip6tables']
def capture_extra():
    values={}
    for k in KEYS:
        p=pathlib.Path('/proc/sys/'+k.replace('.','/'))
        if p.exists():values[k]=p.read_text().strip()
    return {'network':network_snapshot(),'sysctl':values,'modules':{m:pathlib.Path('/sys/module/'+m).exists() for m in ['overlay','br_netfilter']}}
def precheck():
    assert not run(['swapon','--show','--noheadings'])
    assert service_state('kubelet')['active']!='active'
    assert not run(['docker','ps','-q']),'Running containers prevent stale cluster network preparation'
def install():
    cleanup_netfilter(['KUBE-','CNI-','FLANNEL-'])
    for m in ['overlay','br_netfilter']:run(['modprobe',m])
    write(PATHS[0],'\n'.join(k+' = 1' for k in KEYS)+'\n',0o644)
    write(PATHS[1],'overlay\nbr_netfilter\n',0o644)
    run(['sysctl','-p',PATHS[0]])
def postcheck():
    for k in KEYS:assert run(['sysctl','-n',k])=='1',k
    assert not run(['swapon','--show','--noheadings'])
def restore_extra(s):
    network_restore(s['network'],['KUBE-','CNI-','FLANNEL-'])
    for k,v in s['sysctl'].items():run(['sysctl','-w',k+'='+v])
    for m,existed in s['modules'].items():
        if not existed and pathlib.Path('/sys/module/'+m).exists():run(['modprobe','-r',m])
def verify_extra(s):
    network_restored(s['network'])
    for k,v in s['sysctl'].items():assert run(['sysctl','-n',k])==v,k
    for m,existed in s['modules'].items():assert pathlib.Path('/sys/module/'+m).exists()==existed,m
