...
COMPONENT='etcd-state-store';PATHS=['/var/lib/etcd','/etc/kubernetes/manifests/etcd.yaml'];SERVICES=[]
def precheck():
    assert IS_MASTER
    assert I['etcd_data_dir']=='/var/lib/etcd'
    for f in ['ca.crt','server.crt','server.key','peer.crt','peer.key']:assert (PKI/'etcd'/f).is_file(),f
    assert service_state('kubelet')['active']=='active'
    if exists('/var/lib/etcd/member') and not MARKER.exists():raise RuntimeError('An existing etcd member cannot be adopted')
def install():
    pathlib.Path('/var/lib/etcd').mkdir(parents=True,exist_ok=True)
    args=['etcd','--name='+NODE,'--data-dir=/var/lib/etcd','--listen-client-urls=https://127.0.0.1:2379,https://'+IP+':2379','--advertise-client-urls=https://'+IP+':2379','--listen-peer-urls=https://'+IP+':2380','--initial-advertise-peer-urls=https://'+IP+':2380','--initial-cluster='+','.join(m+'=https://'+HOSTS[m]+':2380' for m in MASTERS),'--initial-cluster-state=new','--initial-cluster-token=clusterforge-k8s1175','--client-cert-auth=true','--trusted-ca-file=/etc/kubernetes/pki/etcd/ca.crt','--cert-file=/etc/kubernetes/pki/etcd/server.crt','--key-file=/etc/kubernetes/pki/etcd/server.key','--peer-client-cert-auth=true','--peer-trusted-ca-file=/etc/kubernetes/pki/etcd/ca.crt','--peer-cert-file=/etc/kubernetes/pki/etcd/peer.crt','--peer-key-file=/etc/kubernetes/pki/etcd/peer.key','--snapshot-count=10000']
    write(PATHS[1],static_pod('etcd',args,[(str(PKI/'etcd'),True),('/var/lib/etcd',False)]),0o600)
def postcheck():
    def health():
        cs=containers('etcd')
        if not cs:return False
        result=run(['docker','exec','-e','ETCDCTL_API=3',cs[0]['Id'],'etcdctl','--endpoints='+','.join('https://'+HOSTS[m]+':2379' for m in MASTERS),'--cacert=/etc/kubernetes/pki/etcd/ca.crt','--cert=/etc/kubernetes/pki/etcd/server.crt','--key=/etc/kubernetes/pki/etcd/server.key','endpoint','health','-w=json'])
        items=json.loads(result);return len(items)==3 and all(x['health'] for x in items)
    wait_for(health)
def rollback_precheck():assert not exists('/etc/kubernetes/manifests/kube-apiserver.yaml'),'API servers must be removed first'
def rollback():
    remove(PATHS[1]);wait_for(lambda:not containers('etcd'))
def rollback_postcheck():assert not containers('etcd')
