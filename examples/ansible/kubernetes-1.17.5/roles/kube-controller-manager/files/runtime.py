...
COMPONENT='kube-controller-manager';PATHS=['/etc/kubernetes/manifests/kube-controller-manager.yaml'];SERVICES=[]
def precheck():assert kubectl(['get','--raw=/healthz'])=='ok'
def install():
    args=['kube-controller-manager','--bind-address=127.0.0.1','--port=10252','--secure-port=10257','--kubeconfig=/etc/kubernetes/controller-manager.conf','--authentication-kubeconfig=/etc/kubernetes/controller-manager.conf','--authorization-kubeconfig=/etc/kubernetes/controller-manager.conf','--leader-elect=true','--use-service-account-credentials=true','--cluster-signing-cert-file='+str(PKI/'ca.crt'),'--cluster-signing-key-file='+str(PKI/'ca.key'),'--root-ca-file='+str(PKI/'ca.crt'),'--service-account-private-key-file='+str(PKI/'sa.key'),'--allocate-node-cidrs=true','--cluster-cidr='+I['pod_network_cidr'],'--service-cluster-ip-range='+I['service_cidr'],'--controllers=*,bootstrapsigner,tokencleaner']
    write(PATHS[0],static_pod(COMPONENT,args,[(str(PKI),True),('/etc/kubernetes/controller-manager.conf',True)]))
def postcheck():
    import urllib.request
    def health():
        with urllib.request.urlopen('http://127.0.0.1:10252/healthz',timeout=3) as r:return r.read()==b'ok'
    wait_for(health)
    wait_for(lambda: bool(get_object('endpoints',COMPONENT,'kube-system')))
def rollback():remove(PATHS[0]);wait_for(lambda:not containers(COMPONENT))
def rollback_postcheck():assert not containers(COMPONENT)
