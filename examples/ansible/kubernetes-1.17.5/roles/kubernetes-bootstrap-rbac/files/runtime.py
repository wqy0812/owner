...
COMPONENT='kubernetes-bootstrap-rbac';PATHS=[];SERVICES=[]
ROLES=[('clusterforge-1175-bootstrap','system:node-bootstrapper','system:bootstrappers:clusterforge1175'),('clusterforge-1175-csr','system:certificates.k8s.io:certificatesigningrequests:nodeclient','system:bootstrappers:clusterforge1175'),('clusterforge-1175-rotation','system:certificates.k8s.io:certificatesigningrequests:selfnodeclient','system:nodes')]
def objects():return [{'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRoleBinding','metadata':{'name':name},'roleRef':{'apiGroup':'rbac.authorization.k8s.io','kind':'ClusterRole','name':role},'subjects':[{'apiGroup':'rbac.authorization.k8s.io','kind':'Group','name':group}]} for name,role,group in ROLES]
def precheck():assert kubectl(['get','--raw=/healthz'])=='ok'
def install():
    if NODE!=PRIMARY:return
    apply_objects(objects())
    for node in MASTERS:
        wait_for(lambda:get_object('node',node) is not None)
...
def postcheck():
    for expected in objects():
        actual=get_object('clusterrolebinding',expected['metadata']['name']);assert actual and actual['roleRef']==expected['roleRef'] and actual['subjects']==expected['subjects']
def rollback():
    if NODE!=PRIMARY:return
    for node in MASTERS:
        if get_object('node',node):
