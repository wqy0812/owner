...
COMPONENT='coredns';PATHS=[];SERVICES=[]
def objects():
    ns='kube-system';name='clusterforge-coredns'
    core='.:53 {\n    errors\n    health\n    ready\n    kubernetes cluster.local in-addr.arpa ip6.arpa {\n        pods insecure\n        fallthrough in-addr.arpa ip6.arpa\n        ttl 30\n    }\n    prometheus :9153\n    forward . /etc/resolv.conf\n    cache 30\n    loop\n    reload\n    loadbalance\n}\n'
    pod={'serviceAccountName':name,'priorityClassName':'system-cluster-critical','dnsPolicy':'Default','containers':[{'name':'coredns','image':image('coredns'),'args':['-conf','/etc/coredns/Corefile'],'resources':{'requests':{'cpu':'100m','memory':'70Mi'},'limits':{'memory':'170Mi'}},'ports':[{'containerPort':53,'name':'dns','protocol':'UDP'},{'containerPort':53,'name':'dns-tcp','protocol':'TCP'},{'containerPort':9153,'name':'metrics','protocol':'TCP'}],'volumeMounts':[{'name':'config','mountPath':'/etc/coredns','readOnly':True}],'livenessProbe':{'httpGet':{'path':'/health','port':8080},'initialDelaySeconds':15},'readinessProbe':{'httpGet':{'path':'/ready','port':8181}}}],'volumes':[{'name':'config','configMap':{'name':name}}]}
    return [{'apiVersion':'v1','kind':'ServiceAccount','metadata':{'name':name,'namespace':ns}},{'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRole','metadata':{'name':name},'rules':[{'apiGroups':[''],'resources':['endpoints','services','pods','namespaces'],'verbs':['list','watch']}]},{'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRoleBinding','metadata':{'name':name},'roleRef':{'apiGroup':'rbac.authorization.k8s.io','kind':'ClusterRole','name':name},'subjects':[{'kind':'ServiceAccount','name':name,'namespace':ns}]},{'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':name,'namespace':ns},'data':{'Corefile':core}},{'apiVersion':'v1','kind':'Service','metadata':{'name':'kube-dns','namespace':ns,'labels':{'k8s-app':'kube-dns'}},'spec':{'clusterIP':I['cluster_dns_ip'],'selector':{'k8s-app':'kube-dns'},'ports':[{'name':'dns','port':53,'protocol':'UDP'},{'name':'dns-tcp','port':53,'protocol':'TCP'}]}},{'apiVersion':'apps/v1','kind':'Deployment','metadata':{'name':name,'namespace':ns},'spec':{'replicas':2,'selector':{'matchLabels':{'k8s-app':'kube-dns'}},'template':{'metadata':{'labels':{'k8s-app':'kube-dns'}},'spec':pod}}}]
def precheck():assert kubectl(['get','--raw=/healthz'])=='ok'
def install():
    if NODE==PRIMARY:apply_objects(objects())
def postcheck():
    wait_for(lambda:get_object('deployment','clusterforge-coredns','kube-system').get('status',{}).get('readyReplicas')==2,240)
    assert get_object('service','kube-dns','kube-system')['spec']['clusterIP']==I['cluster_dns_ip']
    wait_for(lambda:sum(len(x.get('addresses',[])) for x in get_object('endpoints','kube-dns','kube-system').get('subsets',[]))>=2)
def rollback():
    if NODE==PRIMARY:delete_objects()
def rollback_postcheck():
    for obj in objects():
        m=obj['metadata']
        assert get_object(obj['kind'],m['name'],m.get('namespace')) is None,'Owned API resource remains: '+m['name']
