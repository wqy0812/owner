...
COMPONENT='flannel';PATHS=['/etc/cni/net.d/10-flannel.conflist', '/run/flannel', '/opt/cni/bin/flannel'];SERVICES=[]
def objects():
    ns='kube-flannel';name='clusterforge-flannel'
    cni={'name':'cbr0','cniVersion':'0.3.1','plugins':[{'type':'flannel','delegate':{'hairpinMode':True,'isDefaultGateway':True}},{'type':'portmap','capabilities':{'portMappings':True}}]}
    cfg={'Network':I['pod_network_cidr'],'Backend':{'Type':'vxlan'}}
    pod={'serviceAccountName':name,'hostNetwork':True,'priorityClassName':'system-node-critical','tolerations':[{'operator':'Exists'}],'initContainers':[{'name':'install-cni-plugin','image':image('flannel_cni_plugin'),'command':['cp','-f','/flannel','/opt/cni/bin/flannel'],'volumeMounts':[{'name':'cni-plugin','mountPath':'/opt/cni/bin'}]},{'name':'install-cni','image':image('flannel'),'command':['cp','-f','/etc/kube-flannel/cni-conf.json','/etc/cni/net.d/10-flannel.conflist'],'volumeMounts':[{'name':'cni','mountPath':'/etc/cni/net.d'},{'name':'config','mountPath':'/etc/kube-flannel'}]}],'containers':[{'name':'kube-flannel','image':image('flannel'),'command':['/opt/bin/flanneld'],'args':['--ip-masq','--kube-subnet-mgr'],'env':[{'name':'POD_NAME','valueFrom':{'fieldRef':{'fieldPath':'metadata.name'}}},{'name':'POD_NAMESPACE','valueFrom':{'fieldRef':{'fieldPath':'metadata.namespace'}}},{'name':'EVENT_QUEUE_DEPTH','value':'5000'}],'securityContext':{'capabilities':{'add':['NET_ADMIN','NET_RAW']}},'resources':{'requests':{'cpu':'100m','memory':'50Mi'}},'volumeMounts':[{'name':'run','mountPath':'/run/flannel'},{'name':'config','mountPath':'/etc/kube-flannel'},{'name':'xtables','mountPath':'/run/xtables.lock'}]}],'volumes':[{'name':'run','hostPath':{'path':'/run/flannel'}},{'name':'config','configMap':{'name':name}},{'name':'cni-plugin','hostPath':{'path':'/opt/cni/bin'}},{'name':'cni','hostPath':{'path':'/etc/cni/net.d'}},{'name':'xtables','hostPath':{'path':'/run/xtables.lock','type':'FileOrCreate'}}]}
    return [{'apiVersion':'v1','kind':'Namespace','metadata':{'name':ns}},{'apiVersion':'v1','kind':'ServiceAccount','metadata':{'name':name,'namespace':ns}},{'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRole','metadata':{'name':name},'rules':[{'apiGroups':[''],'resources':['pods'],'verbs':['get']},{'apiGroups':[''],'resources':['nodes'],'verbs':['get','list','watch']},{'apiGroups':[''],'resources':['nodes/status'],'verbs':['patch']}]},{'apiVersion':'rbac.authorization.k8s.io/v1','kind':'ClusterRoleBinding','metadata':{'name':name},'roleRef':{'apiGroup':'rbac.authorization.k8s.io','kind':'ClusterRole','name':name},'subjects':[{'kind':'ServiceAccount','name':name,'namespace':ns}]},{'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':name,'namespace':ns},'data':{'cni-conf.json':json.dumps(cni),'net-conf.json':json.dumps(cfg)}},{'apiVersion':'apps/v1','kind':'DaemonSet','metadata':{'name':name,'namespace':ns},'spec':{'selector':{'matchLabels':{'app':name}},'template':{'metadata':{'labels':{'app':name}},'spec':pod}}}]
def precheck():
    for plugin in ['bridge','host-local','loopback','portmap']:
        p=pathlib.Path('/opt/cni/bin')/plugin
        assert p.is_file() and os.access(str(p),os.X_OK),'Missing base CNI plugin: '+plugin
    assert service_state('kubelet')['active']=='active'
    if not MARKER.exists():
        for name in ['cni0','flannel.1']:assert not pathlib.Path('/sys/class/net/'+name).exists(),'Unowned network interface '+name
def install():
    pathlib.Path('/etc/cni/net.d').mkdir(parents=True,exist_ok=True)
    if NODE==PRIMARY:apply_objects(objects())
def postcheck():
    wait_for(lambda:check_container('kube-flannel'))
    wait_for(lambda:pathlib.Path('/etc/cni/net.d/10-flannel.conflist').exists())
    assert json.loads(pathlib.Path('/etc/cni/net.d/10-flannel.conflist').read_text())['plugins'][0]['type']=='flannel'
    if NODE==PRIMARY:wait_for(lambda: get_object('daemonset','clusterforge-flannel','kube-flannel').get('status',{}).get('numberReady')==6)
def rollback():
    if NODE==PRIMARY:delete_objects()
    wait_for(lambda:not containers('kube-flannel'))
    for name in ['cni0','flannel.1']:
        if pathlib.Path('/sys/class/net/'+name).exists():run(['ip','link','delete',name])
    cleanup_netfilter(['FLANNEL-','CNI-'])
def rollback_postcheck():
    assert not containers('kube-flannel')
    for name in ['cni0','flannel.1']:assert not pathlib.Path('/sys/class/net/'+name).exists()
    if NODE==PRIMARY:objects_absent()
