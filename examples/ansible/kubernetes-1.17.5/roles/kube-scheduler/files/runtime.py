...
COMPONENT='kube-scheduler';PATHS=['/etc/kubernetes/manifests/kube-scheduler.yaml'];SERVICES=[]
def precheck():assert kubectl(['get','--raw=/healthz'])=='ok'
def install():
    args=['kube-scheduler','--bind-address=127.0.0.1','--port=10251','--secure-port=10259','--kubeconfig=/etc/kubernetes/scheduler.conf','--authentication-kubeconfig=/etc/kubernetes/scheduler.conf','--authorization-kubeconfig=/etc/kubernetes/scheduler.conf','--leader-elect=true']
    write(PATHS[0],static_pod(COMPONENT,args,[(str(PKI),True),('/etc/kubernetes/scheduler.conf',True)]))
def postcheck():
    import urllib.request
    def health():
        with urllib.request.urlopen('http://127.0.0.1:10251/healthz',timeout=3) as r:return r.read()==b'ok'
    wait_for(health)
