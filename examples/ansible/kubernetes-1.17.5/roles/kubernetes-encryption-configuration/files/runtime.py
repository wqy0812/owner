...
COMPONENT='kubernetes-encryption-configuration';PATHS=['/etc/kubernetes/encryption-config.yaml'];SERVICES=[]
def precheck():
    key=os.environ['CF_ENCRYPTION_KEY'];assert len(base64.b64decode(key,validate=True))==32,'Expected 32-byte encryption key'
    assert not exists(PATHS[0]) or MARKER.exists(),'Existing encryption configuration is not owned by this Run'
def install():
    precheck();write(PATHS[0],{'apiVersion':'apiserver.config.k8s.io/v1','kind':'EncryptionConfiguration','resources':[{'resources':['secrets'],'providers':[{'aescbc':{'keys':[{'name':'clusterforge-key-1','secret':os.environ['CF_ENCRYPTION_KEY']}]}},{'identity':{}}]}]})
def postcheck():
    d=json.loads(pathlib.Path(PATHS[0]).read_text());assert d['resources'][0]['providers'][0]['aescbc']['keys'][0]['secret']==os.environ['CF_ENCRYPTION_KEY']
    assert stat.S_IMODE(pathlib.Path(PATHS[0]).stat().st_mode)==0o600
def rollback_precheck():assert not exists('/etc/kubernetes/manifests/kube-apiserver.yaml'),'API server must stop first'
