...
COMPONENT='docker-runtime';PATHS=[];SERVICES=['docker']
def precheck():
    assert run(['docker','version','--format','{{.Server.Version}}'])==I['docker_version']
    assert run(['docker','info','--format','{{.DockerRootDir}}'])==I['docker_data_dir']
    assert run(['docker','info','--format','{{.CgroupDriver}}'])=='cgroupfs'
def install():
    precheck();run(['systemctl','enable','docker']);run(['systemctl','start','docker'])
def postcheck():
    precheck();assert service_state('docker')['active']=='active'
