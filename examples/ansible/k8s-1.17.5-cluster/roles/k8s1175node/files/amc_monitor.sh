#!/bin/bash
BASE_DIR=$1
#connect to amc with 5 times retry
retryCount=0
while true
do
    echo "starting connecting to AMC"
    exec 8<>/dev/udp/${ENV_AMC_USABLITY_ADDR}/${ENV_AMC_USABLITY_PORT}
    if [ $? -eq 0 ];then
        echo "connecting to AMC succeed"
        break
    else
        retryCount=$[${retryCount}+1]
        if [ ${count} -gt ${MaxRetryCount} ];then
            echo "exceed the retry count limit"
            exit 1
        fi
        echo "connect to AMC failed, will retry in ${RetryInterval}.."
        sleep ${RetryInterval}
    fi
done
