...
find "$snapshot_root/components" -type f -name '*.yml' -print | xargs -I '{}' -P 4 sh -c '
  ...
  roles=$2
  inventory=$3
  ANSIBLE_ROLES_PATH="$roles" "$ANSIBLE_PLAYBOOK" -i "$inventory" --syntax-check "$playbook" >/dev/null
  ANSIBLE_ROLES_PATH="$roles" "$ANSIBLE_PLAYBOOK" -i "$inventory" --list-tasks "$playbook" >/dev/null
' sh '{}' "$roles" "$inventory"

if ANSIBLE_ROLES_PATH="$roles" "$ANSIBLE_PLAYBOOK" -i "$inventory" \
