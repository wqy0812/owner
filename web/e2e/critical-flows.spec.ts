...
test('identity switch changes owner-specific component controls', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/session/me')) data = current;
      ...
      current = users[id];
      data = current;
    } else if (path.endsWith('/components')) data = [{ id: 'containerd', name: 'containerd', ownerId: 'component-alice', kind: 'atomic', latestRelease: { id: 'containerd-2', componentId: 'containerd', version: 'v2.1.1', status: 'released' } }];
...
test('scenario rollback and declared run input are submitted to the API', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.endsWith('/session/me')) data = users['scenario-carol'];
      ...
      id: 'demo-agent', name: 'Demo Node Agent', ownerId: 'component-alice',
      latestRelease: {
        id: 'demo-agent-1.1', componentId: 'demo-agent', version: 'v1.1.0', status: 'released', verified: true,
      ...
      revisions: [{ id: 'safe-upgrade-r2', scenarioId: 'safe-upgrade', revision: 2, status: 'draft', graph: {
        nodes: [{ id: 'agent', name: 'Rollback agent', releaseId: 'demo-agent-1.1', action: 'rollback', hostGroup: 'demo_nodes', runInputs: ['rollback_version'], position: { x: 80, y: 80 } }], edges: [],
      } }],
    }];
    else if (path.endsWith('/environments')) data = [{ id: 'local', name: 'Localhost', ownerId: 'environment-dave', currentRevision: { id: 'local-r1', revision: 1, hosts: [] } }];
    else if (path.endsWith('/test-runs')) {
      submitted = request.postDataJSON();
      data = { id: 'run-1', status: 'queued', environmentId: 'local' };
    }
    await route.fulfill({ status: path.endsWith('/test-runs') ? 202 : 200, contentType: 'application/json', body: JSON.stringify({ data }) });
  });

  await page.goto('/scenarios');
  await page.getByText('Rollback agent').click();
  ...
  await page.getByRole('combobox', { name: '共享测试环境' }).selectOption('local');
  await page.getByLabel('运行参数 rollback_version').fill('1.0.0');
  await page.getByRole('button', { name: '开始完整测试' }).click();
  await expect.poll(() => submitted).toEqual({ environmentId: 'local', runInput: { rollback_version: '1.0.0' } });
});
