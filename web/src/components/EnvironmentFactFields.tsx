...
export function EnvironmentFactFields({ categories, facts, editable, onChange }: { categories: PlatformOptionCategory[]; facts: Record<string, unknown>; editable: boolean; onChange: (facts: Record<string, unknown>) => void }) {
  ...
  return <div className="form-grid">{dimensions.map((dimension) => {
    ...
    return <label key={dimension.key}><span>{dimension.label}{category?.environmentRequired ? '（必填）' : '（可选）'}</span><select aria-label={`环境标签 ${dimension.label}`} required={category?.environmentRequired} value={currentValue} disabled={!editable || Boolean(parent && !parentValue)} onChange={(event) => setFact(dimension.key, event.target.value)}><option value="">{parent && !parentValue ? `请先选择${parent.label}` : '未选择'}</option>{currentRetired ? <option value={currentRetired.value} disabled>{currentRetired.label}（已退役，只读）</option> : null}{activeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>;
