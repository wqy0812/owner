...
export function ScenarioCreateModal({ scenarios, initialSourceRevisionId = '', onClose, onDone }: {
    scenarios: Scenario[];
    initialSourceRevisionId?: string;
    onClose: () => void;
    onDone: (scenario: Scenario) => void;
}) {
    ...
    const [plan, setPlan] = useState<ScenarioForkPlan>();
    ...
    async function changeSource(id: string) {
        if (scopeDirty && !await confirm('切换来源会用来源标签替换当前选择，确认继续？'))
            return;
        setSourceRevisionId(id);
        setScope(id ? sources.find(item => item.revision.id === id)?.revision.environmentConstraints ?? {} : undefined);
        setScopeDirty(false);
        setConfirmed(false);
        invalidate();
    }
    async function submit(_values?: unknown) {
        if (busy || !confirmed || !scopeReady)
            return;
        setBusy(true);
        try {
            const input = { sourceRevisionId, name, slug, description, environmentConstraints };
            if (mode === 'fork' && !plan) {
                setPlan(await api.previewScenarioFork(input));
                return;
            }
            ...
            notify('success', mode === 'fork' ? '分支已创建' : '场景已创建', '从独立 r1 草稿开始；适配范围已固定。');
            onDone(created);
        }
        catch (reason) {
    ...
    return <Modal busy={Boolean(busy)} size="wide" title={mode === 'fork' ? '新增分支' : '新建场景'} description="创建时确定适配标签；创建后如需改变范围，请新增分支。" onClose={onClose} formProps={{ onFinish: event => submit(event), layout: "vertical", preserve: false }} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--primary" disabled={busy || !confirmed || !scopeReady} htmlType={"submit"} type="primary">{busy ? '处理中…' : mode === 'fork' ? plan ? '确认创建分支' : '预览分支' : '创建场景'}</Button></footer>}><div className="form-grid">
    {mode === 'fork' && <Field className="span-2" label={"来源场景与已发布版本"} required><Select aria-label="分支来源版本" value={sourceRevisionId} onChange={(selectedValue) => changeSource(selectedValue)} popupMatchSelectWidth={true}><Select.Option value="">请选择已发布版本</Select.Option>{sources.map(({ scenario, revision }) => <Select.Option key={revision.id} value={revision.id}>{scenario.name} · r{revision.revision} · {scenario.ownerName ?? '场景 Owner'}</Select.Option>)}</Select></Field>}
    <Field label={"场景名称"} name={"name"} required rules={[{ required: true, message: "请填写此项" }]}><Input required value={name} onChange={event => { setName(event.target.value); invalidate(); }} placeholder="例如 Kylin Kubernetes 集群"/></Field>
    <Field label={"标识"} name={"slug"} required rules={[{ required: true, message: "请填写此项" }, { pattern: new RegExp("^(?:" + "[a-z0-9]+(?:-[a-z0-9]+)*" + ")$"), message: "格式不正确" }]}><Input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={slug} onChange={event => { setSlug(event.target.value); invalidate(); }} placeholder="kylin-k8s-cluster"/></Field>
    <Field className="span-2" label={"说明"} name={"description"}><Input.TextArea rows={3} value={description} onChange={event => { setDescription(event.target.value); invalidate(); }}/></Field>
    <div className="span-2"><EnvironmentConstraintEditor value={constraints} title="分支支持范围" onChange={next => { setScope(next); setScopeDirty(true); setConfirmed(false); invalidate(); }}/><div className="scope-confirmation"><BranchScope scope={environmentConstraints} selection={constraints} full/>{unselected.length > 0 && <small>请先选择：{unselected.map(dimension => dimension.label).join('、')}。</small>}<Checkbox className="checkbox-field" required checked={confirmed} disabled={!scopeReady} onChange={event => setConfirmed(event.target.checked)}><span>确认以上适配范围，创建后固定。</span></Checkbox></div></div>
