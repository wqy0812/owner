...
export function ScenarioAcceptanceEditor({ revisionId, editable, nodes, components, environments, onSaved }: {
    ...
    useEffect(() => {
        ...
        setFileDirty(false);
        void api.scenarioAcceptance(revisionId, controller.signal).then(apply).catch(reason => {
    ...
    async function saveDefinition() {
        ...
        setBusy(true);
        try {
            apply(await api.saveScenarioAcceptance(revisionId, { jobs: jobs.map(job => ({ ...job, requiredCredentials: job.requiredCredentials.map(name => name.trim()).filter(Boolean) })), parameters, values, bindings, expectedRevisionDigest: definition.revisionDigest }));
            onSaved();
    ...
    async function saveFile(path: string, text: string, file?: File) {
        ...
        try {
            ...
            apply(await api.scenarioAcceptance(revisionId));
    ...
    async function deleteFile() {
        if (!definition || !selectedFile || dirty || !await confirm(`删除验收工作区文件 ${selectedFile.path}？`))
            return;
        setBusy(true);
        try {
    ...
    if (error)
        return <ErrorBlock message={error} onRetry={() => { setError(undefined); void api.scenarioAcceptance(revisionId).then(apply).catch(reason => setError(displayError(reason))); }}/>;
    if (!definition)
    ...
    <div className="scenario-acceptance-jobs">{jobs.map((job, index) => <article className="scenario-acceptance-job" key={job.id}>
      ...
      <fieldset disabled={!canEdit || busy} className="form-grid">
        ...
        <Field label={"目标主机组"}><Select aria-label={`验收主机组 ${index + 1}`} value={job.hostGroup} onChange={(selectedValue) => updateJob(index, { hostGroup: selectedValue })} popupMatchSelectWidth={true}><Select.Option value="">请选择</Select.Option>{hostGroups.map(group => <Select.Option key={group.id} value={group.value}>{group.label}</Select.Option>)}{job.hostGroup && !hostGroups.some(group => group.value === job.hostGroup) && <Select.Option value={job.hostGroup}>{job.hostGroup}（需复核）</Select.Option>}</Select></Field>
        ...
        <Field label={"风险等级"}><Select value={job.riskLevel} onChange={(selectedValue) => updateJob(index, { riskLevel: selectedValue as ScenarioAcceptanceJob['riskLevel'] })} popupMatchSelectWidth={true}><Select.Option value="low">低风险</Select.Option><Select.Option value="medium">中风险</Select.Option><Select.Option value="high">高风险</Select.Option><Select.Option value="destructive">破坏性操作</Select.Option></Select></Field>
        ...
        <div className="scenario-checkboxes"><Checkbox checked={job.become} onChange={event => updateJob(index, { become: event.target.checked })}> 提权执行</Checkbox><Checkbox checked={job.mayMutate} onChange={event => updateJob(index, { mayMutate: event.target.checked })}> 允许临时业务操作</Checkbox></div>
      </fieldset>
      ...
      <div className="scenario-inline-actions"><StatusPill status={job.playbookSha256 ? 'ready' : 'pending'}>{job.playbookSha256 ? '已录入 Ansible 任务' : '待录入 Ansible 任务'}</StatusPill><Button className="button button--secondary" disabled={busy || dirty || !job.playbook} onClick={() => {
                ...
                if (definition.workspace.files.some(file => file.path === path))
                    ...
                    setSelectedFile({ releaseId: revisionId, path, sha256: '', sizeBytes: 0, mediaType: 'text/yaml', editable: true, content: '' });
                    setContent('- name: 验证业务响应\n  ansible.builtin.uri:\n    url: "{{ acceptance_url }}"\n    status_code: 200\n');
                    setFileDirty(true);
                }
    ...
    <section className="scenario-acceptance-parameters"><header className="scenario-section-title"><span className="panel__icon"><Settings2 size={18} aria-hidden="true"/></span><div><h3>验收参数</h3><p>录入类型化值，或绑定组件公开参数及环境字段。</p></div></header>{!parameters.length && <EmptyState title="尚未配置验收参数" description="需要传入业务地址等信息时，可添加参数并选择值的来源。"/>}{parameters.map((parameter, index) => {
        ...
        <Field label={"类型"}><Select value={parameter.type} onChange={(selectedValue) => { updateParameter(index, { type: selectedValue as ParameterDefinition['type'], enum: undefined }); setBinding(parameter.name); }} popupMatchSelectWidth={true}><Select.Option value="string">文本</Select.Option><Select.Option value="boolean">开关</Select.Option><Select.Option value="integer">整数</Select.Option><Select.Option value="number">数值</Select.Option>{['array', 'object'].includes(parameter.type) && <Select.Option value={parameter.type}>{parameter.type}（受控选项）</Select.Option>}</Select></Field>
        <Field label={"值的来源"}><Select value={binding?.source ?? 'value'} onChange={(selectedValue) => setBinding(parameter.name, selectedValue === 'value' ? undefined : { parameter: parameter.name, source: selectedValue as 'node' | 'environment', sourceParameter: '' })} popupMatchSelectWidth={true}><Select.Option value="value">场景 Owner 填写</Select.Option><Select.Option value="node">节点公开参数</Select.Option><Select.Option value="environment">环境字段</Select.Option></Select></Field>
        <Field className="span-2" label={binding ? '绑定来源' : '参数值'}>{binding?.source === 'node' ? <Select value={binding.nodeId && binding.sourceParameter ? `${binding.nodeId}::${binding.sourceParameter}` : ''} onChange={(selectedValue) => { const [nodeId, sourceParameter] = selectedValue.split('::'); setBinding(parameter.name, { ...binding, nodeId, sourceParameter }); }} popupMatchSelectWidth={true}><Select.Option value="">请选择同类型公开参数</Select.Option>{sourceOptions.map(option => <Select.Option key={option.value} value={option.value}>{option.label}</Select.Option>)}{binding.sourceParameter && !sourceOptions.some(option => option.value === `${binding.nodeId}::${binding.sourceParameter}`) && <Select.Option value={`${binding.nodeId}::${binding.sourceParameter}`}>失效来源 · {binding.sourceParameter}</Select.Option>}</Select> : binding?.source === 'environment' ? <Select value={binding.sourceParameter} onChange={(selectedValue) => setBinding(parameter.name, { ...binding, sourceParameter: selectedValue })} popupMatchSelectWidth={true}><Select.Option value="">请选择环境字段</Select.Option>{environmentNames.map(name => <Select.Option key={name} value={name}>{name}</Select.Option>)}{binding.sourceParameter && !environmentNames.includes(binding.sourceParameter) && <Select.Option value={binding.sourceParameter}>{binding.sourceParameter}（当前环境未提供）</Select.Option>}</Select> : <ParameterValueEditor parameter={parameter} value={values[parameter.name]} disabled={!canEdit} optional={!parameter.required} onChange={value => {
                        setValues(current => {
                            const next = { ...current };
                            if (value === undefined)
        ...
        <Checkbox className="scenario-checkboxes" checked={parameter.required === true} onChange={event => updateParameter(index, { required: event.target.checked })}> 必填</Checkbox>
      </fieldset>{canEdit && <Button className="button button--danger-soft" onClick={() => { setParameters(parameters.filter((_, i) => i !== index)); setBinding(parameter.name); setDirty(true); }} htmlType={"button"} type="default" danger><Trash2 size={15}/> 删除参数</Button>}</article>;
