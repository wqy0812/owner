...
export function RunDeliveryPanel({run}:{run:Run}){
 ...
 return <article className="panel execution-delivery">
  ...
  <div className="delivery-options">
   <section className="delivery-card"><header><FileCheck2 size={18} aria-hidden="true" /><h3>本次作业包</h3><StatusPill status="locked">已锁定</StatusPill></header><p className="delivery-scope">{run.retryOfRunId?'本次续跑选择的阶段':'本次锁定的执行范围'} · {run.purposeCounts?.total??run.steps?.length??0} 步</p><Button className="button button--secondary" href={api.runJobDownloadURL(run.id)} type="default"><Download size={14} aria-hidden="true" />下载本次作业包</Button></section>
   <section className={`delivery-card${value?.eligible ? ' delivery-card--verified' : ''}`}><header><ShieldCheck size={18} aria-hidden="true" /><h3>已验证完整作业</h3><StatusPill status={value?.eligible ? 'succeeded' : error ? 'failed' : 'pending'}>{value?.eligible ? '可交付' : error ? '核验失败' : value ? '待满足条件' : '核验中'}</StatusPill></header>{value?.eligible?<><p className="delivery-scope">根作业：{value.rootRunId} · 完整流程 {value.stageCount} 步</p><p className="delivery-evidence">证据链：{value.evidenceRunIds.join(' → ')}</p><Button className="button button--primary" href={api.runJobDownloadURL(run.id,true)} type="primary"><Download size={14} aria-hidden="true" />下载已验证完整作业</Button></>:<div role="status"><InfoNote>{error||value?.reason||'正在核验完整流程、合同和证据链…'}</InfoNote></div>}</section>
  </div>
 </article>
}
