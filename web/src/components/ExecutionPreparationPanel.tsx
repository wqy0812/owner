...
export function ExecutionPreparationPanel({ request, disabled, onPlan }: {
    request: PreparationRequest;
    disabled?: boolean;
    onPlan: (plan: ComponentTestPlan | ScenarioExecutionPreview | undefined) => void;
}) {
    const { user } = useApp();
        ...
        if (stored) {
            try {
                ...
                if (saved.signature === signature)
                    void api.preparation(saved.id).then(value => { if (epoch.current === generation)
                        apply(value); }).catch(reason => { if (epoch.current === generation)
                        setError(displayError(reason)); });
            }
            catch {
                sessionStorage.removeItem(`preparation:${scope}`);
            }
        }
        return () => { epoch.current++; };
    }, [signature, scope, apply]);
    useEffect(() => {
        if (!session || !['queued', 'running'].includes(session.status))
            return;
        let cancelled = false;
        const timer = window.setInterval(() => { void api.preparation(session.id).then(value => { if (!cancelled) {
            apply(value);
            setError('');
        } }).catch(reason => { if (!cancelled)
            setError(displayError(reason)); }); }, 1500);
    ...
    const start = async () => {
        ...
        delivered.current = '';
        try {
            const value = await api.createPreparation({ ...request, idempotencyKey: newClientID() });
            if (generation !== epoch.current)
                return;
            sessionStorage.setItem(`preparation:${scope}`, JSON.stringify({ id: value.id, signature }));
            apply(value);
        }
        catch (reason) {
            if (generation === epoch.current)
    ...
    return <section className="execution-preparation" aria-label="执行准备"><header className="execution-preparation__header"><div className="editor-section-heading"><span className="panel__icon"><ClipboardCheck size={18} aria-hidden="true"/></span><div><h4>执行准备</h4><p>逐项检查并生成锁定计划；页面刷新后可以继续查看进度。</p></div></div><div className="scenario-inline-actions"><Button className="button button--secondary" disabled={disabled || busy || active || !request.environmentId} onClick={() => void start()} htmlType={"button"} type="default">{busy ? '正在创建准备任务…' : session ? '重新准备执行计划' : '预览执行计划'}</Button>{active && <Button className="button button--quiet" onClick={() => void api.cancelPreparation(session!.id).catch(reason => setError(displayError(reason)))} htmlType={"button"} type="default">取消准备</Button>}</div></header>
