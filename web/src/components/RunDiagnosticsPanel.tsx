...
function DiagnosticDetails({ item, onLocateStep }: {
    item: RunDiagnostic;
    ...
    {item.stepId && <Button className="button button--quiet" onClick={() => onLocateStep(item.stepId!)} htmlType={"button"} type="default">定位失败步骤</Button>}
  </div>;
}
export function RunDiagnosticsPanel({ run, retryBusy, onRetryRun, onLocateStep }: {
    run: Run;
    retryBusy: boolean;
    onRetryRun: () => void;
    onLocateStep: (stepId: string) => void;
    ...
    if (!enabled)
        return null;
    if (query.error)
        return <section className="panel run-diagnostic-status"><ErrorBlock message={`诊断日志读取失败：${query.error}`} onRetry={() => void query.reload()}/></section>;
    if (!query.data)
        return <section className="panel run-diagnostic-status" role="status">正在读取关键错误…</section>;
    ...
    <div className="run-diagnostic__body">
      ...
      <details><summary>展开完整诊断</summary><DiagnosticDetails item={primary} onLocateStep={onLocateStep}/></details>
      {others.length > 0 && <details><summary>其他失败记录 · {others.length}</summary>{others.map((item, index) => <section className="run-diagnostic__other" key={`${item.logId}-${index}`}><strong>{item.host || item.component || '执行错误'}</strong><p>{item.message}</p><DiagnosticDetails item={item} onLocateStep={onLocateStep}/></section>)}</details>}
      {!!query.data.omitted && <p>另有 {query.data.omitted} 条错误记录，请在完整日志包中查看。</p>}
      <div className="run-diagnostic__actions">
        <Button className="button button--quiet" onClick={async () => { try {
        await navigator.clipboard.writeText(`Run ${run.id}\n${query.data!.items.map(diagnosticText).join('\n\n')}`);
        notify('success', '诊断已复制');
    }
    catch (error) {
...
export function RunLogDownload({ run }: {
    run: Run;
}) {
    ...
    async function download() {
        ...
        try {
            ...
            link.href = url;
            link.download = `${run.id}-logs.tar.gz`;
    ...
    <Button className="button button--secondary" disabled={busy} onClick={() => void download()} htmlType={"button"} type="default"><Download size={15}/>{busy ? '正在生成日志包…' : error ? '重试下载完整日志包' : '下载完整日志包'}</Button>
