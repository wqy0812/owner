...
export function EnvironmentRevisionDeletionModal({ environment, revision, onClose, onDeleted }: {
    ...
    useEffect(() => {
        ...
        void api.environmentRevisionDeletionImpact(environment.id, revision.id, controller.signal).then(value => {
            ...
            if (!controller.signal.aborted)
                setError({ message: displayError(reason), deleting: false });
        }).finally(() => {
            if (!controller.signal.aborted)
                setLoading(false);
        });
    ...
    async function remove() {
        if (!confirmed || !impact?.canDelete || loading || submitting || error)
            return;
        setSubmitting(true);
        try {
    ...
    return <Modal busy={Boolean(submitting)} title="删除环境版本" description={`${environment.name} · r${revision.revision}`} onClose={() => { if (!submitting)
        onClose(); }} footer={<footer className="modal-actions">
      <Button className="button button--quiet" disabled={submitting} onClick={onClose} htmlType={"button"} type="default">取消</Button>
      <Button className="button button--danger" disabled={loading || submitting || !impact?.canDelete || !confirmed || Boolean(error)} onClick={() => void remove()} htmlType={"button"} type="default" danger>{submitting ? '正在删除…' : '确认删除版本'}</Button>
    </footer>}>
      ...
      {loading ? <LoadingBlock label="正在核对版本引用与连通性检查…"/> : <>
        ...
        {impact && <>
          ...
          {impact.canDelete ? <>
            ...
            <Checkbox className="checkbox-field" checked={confirmed} disabled={submitting || Boolean(error)} onChange={event => setConfirmed(event.target.checked)}><span>我确认永久删除 {environment.name} 的 r{revision.revision} 版本</span></Checkbox>
