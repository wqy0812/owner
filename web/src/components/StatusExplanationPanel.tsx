...
function ReasonRow({ reason, currentResourceHref }: { reason: WorkReason; currentResourceHref?: string }) {
  ...
  return <li>
    ...
    <div className="status-explanation__reason-actions">
      {evidenceHref && evidenceHref !== currentResourceHref && nextAction?.href !== evidenceHref && <RouteButton className="icon-text" to={evidenceHref} type="text">查看证据 Run</RouteButton>}
      {nextAction && <RouteButton className="icon-text icon-text--primary" to={nextAction.href} type="primary">{nextAction.label}<ArrowRight size={13} /></RouteButton>}
    </div>
  </li>;
}
  ...
  return <article className={`panel status-explanation status-explanation--${item?.priority ?? 'high'}${reasonsExpanded ? ' status-explanation--expanded' : ''}`} role={item?.priority === 'critical' ? 'alert' : undefined}>
    <header>
      ...
      <div className="status-explanation__header-actions">
        {primaryAction && <RouteButton className="button button--primary" to={primaryAction.href} type="primary">{primaryAction.label}<ArrowRight size={14} /></RouteButton>}
    ...
    {reasonsExpanded && secondaryActions.length > 0 && <footer>{secondaryActions.map((action) => <RouteButton className="icon-text" key={`${action.href}-${action.label}`} to={action.href} type="text">{action.label}</RouteButton>)}</footer>}
  </article>;
}
