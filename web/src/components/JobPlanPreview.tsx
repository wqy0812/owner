...
export function JobPlanPreview({ plan }: { plan: ComponentTestPlan }) {
  return <section className="test-plan-preview" aria-label="完整执行计划">
    <header><div className="test-plan-preview__heading"><span className="panel__icon"><ListOrdered size={17} aria-hidden="true" /></span><div><strong>单作业执行计划</strong><small>环境版本 {plan.environmentRevisionId}</small></div></div><StatusPill status={plan.requiresApproval ? 'awaiting_approval' : 'ready'}>{plan.requiresApproval ? '需环境 Owner 审批' : '可排队执行'}</StatusPill></header>
