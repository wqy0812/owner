...
export function RunCleanupModal({ runIds, onClose, onDeleted }: {
    runIds: string[];
    onClose: () => void;
    onDeleted: (ids: string[]) => void;
}) {
    const { user, notify, signalRefresh } = useApp();
    const { data: preview, loading, error, reload } = useApiData<CleanupItem[]>(() => api.cleanupPreview(runIds), [user.id, runIds.join(':')], []);
    ...
    return <Modal busy={Boolean(busy)} title="删除失败记录" description="仅可删除超过保留期限且未被引用的失败记录。日志和执行详情删除后无法恢复，最小历史与审计记录会保留。" onClose={() => { if (!busy)
        onClose(); }} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={busy} onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--danger-soft" disabled={busy || loading || Boolean(error) || !eligible} onClick={() => void remove()} htmlType={"button"} type="default" danger>{busy ? '删除中…' : '确认删除'}</Button></footer>}>
