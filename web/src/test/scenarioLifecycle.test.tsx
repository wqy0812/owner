...
import { changedReleaseDependencyIssues, changedReleaseParameterIssues, moveAcceptanceJob, newAcceptanceJob, replaceScenarioNodeRelease } from '../features/scenarios/model';
...
describe('scenario lifecycle editor', () => {
  ...
  it('allows the Owner to preview baseline verification for an unpublished test deployment without submitting a test', async () => {
    ...
    await selectOption(screen.getByLabelText('场景执行方式'), '恢复后基线复核');
    await selectOption(screen.getByLabelText('场景执行环境'), '隔离环境');
  ...
  it('requires a current preview and submits the selected mode with digest and an idempotency key', async () => {
    ...
    await selectOption(screen.getByLabelText('场景执行环境'), '隔离环境'); fireEvent.click(screen.getByRole('button', { name: '预览执行计划' })); await screen.findByText('计划已就绪');
  ...
  it('blocks execution when preview reports a baseline mismatch', async () => {
    ...
    render(<ScenarioExecutionPanel revision={revision} environments={[environment]} editable canLaunch initialMode="upgrade" onSaved={vi.fn()} />);
    await selectOption(screen.getByLabelText('场景执行环境'), '隔离环境'); fireEvent.click(screen.getByRole('button', { name: '预览执行计划' })); await screen.findByText('环境实际版本与来源不一致');
    expect(screen.getByRole('button', { name: '提交升级测试' })).toBeDisabled();
  });
