...
import { EmptyState, ErrorBlock, LoadingBlock, PageHeader, formatTime } from '../components/Primitives';
import { displayError, useApp } from '../context/AppContext';
import { useApiData } from '../hooks/useApiData';

export function NotificationsPage() {
  const { user, notify, signalRefresh } = useApp();
  const { data: notifications, loading, error, reload } = useApiData(() => api.notifications(), [user.id]);
  const [filter, setFilter] = useState<'all' | 'unread'>('all');
  const visible = useMemo(() => (notifications ?? []).filter((item) => filter === 'all' || !item.read), [filter, notifications]);

  async function mark(id: string) {
    try { await api.markNotificationRead(id); signalRefresh(); }
    catch (reason) { notify('error', '更新通知失败', displayError(reason)); }
  }

  async function markAll() {
    try { await Promise.all((notifications ?? []).filter((item) => !item.read).map((item) => api.markNotificationRead(item.id))); notify('success', '全部标记为已读'); signalRefresh(); }
    catch (reason) { notify('error', '更新通知失败', displayError(reason)); }
  }
