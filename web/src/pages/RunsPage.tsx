      ...
      {detailLoading && !detail ? <section className="panel"><LoadingBlock label="正在加载运行详情…"/></section> : detailError ? <section className="panel"><ErrorBlock message={detailError} onRetry={() => void reloadDetail()}/></section> : detail ? <section className="run-detail detail-stack">
        <article className="panel run-hero">
          ...
          <div className="run-actions">{user.role === 'platform_admin' && detail.status === 'failed' && <Button className="button button--danger-soft" onClick={() => setCleanupIds([detail.id])} htmlType={"button"} type="default" danger><Trash2 size={14}/> 删除失败记录</Button>}{user.role === 'platform_admin' && detail.status === 'succeeded' && !['archived', 'queued', 'running'].includes(detail.archive?.status ?? '') ? <Button className="button button--quiet" disabled={busy === 'archive'} onClick={async () => {
                    ...
                    }} htmlType={"button"} type="default">归档</Button> : null}<StatusPill status={detail.status}/>{ACTIVE.has(detail.status) && detail.status !== 'awaiting_approval' && <Button disabled={busy === 'cancel'} className="button button--danger-soft" onClick={() => void action('cancel')} htmlType={"button"} type="default" danger><Square size={14}/> 取消</Button>}</div>
