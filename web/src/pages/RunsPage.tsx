...
export function RunsPage() {
    ...
    const [stepToLocate, setStepToLocate] = useState<string>();
    const actionInFlight = useRef(false);
    const logRef = useRef<HTMLPreElement>(null);
    const selectedId = searchParams.get('selected') ?? runs?.[0]?.id;
    const { data: detail, loading: detailLoading, error: detailError, reload: reloadDetail, setData: setDetail } = useApiData<Run | undefined>((signal) => selectedId ? api.run(selectedId, signal) : Promise.resolve(undefined), [user.id, selectedId], 'runs');
    useEffect(() => {
        setActionExplanation(undefined);
        setDeliveryModes({});
    }, [selectedId]);
    useEffect(() => { setDetailTab('steps'); setStepToLocate(undefined); }, [selectedId, user.id]);
    useEffect(() => {
        if (detailTab !== 'steps' || !stepToLocate || detail?.id !== selectedId) return;
        const frame = requestAnimationFrame(() => {
            document.getElementById(`run-step-${stepToLocate}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
            setStepToLocate(undefined);
        });
        return () => cancelAnimationFrame(frame);
    }, [detailTab, stepToLocate, detail?.id, selectedId]);
    ...
    function handleDeletedRuns(ids: string[]) {
        ...
        if (remaining)
            next.set('selected', remaining.id);
        else
            next.delete('selected');
        setSearchParams(next, { replace: true });
    }
    async function action(kind: 'cancel' | 'approve' | 'reject') {
        if (!detail || actionInFlight.current)
            return;
        ...
        try {
            ...
            if (kind === 'cancel')
                updated = await api.cancelRun(detail.id);
            else if (detail.approval?.id)
                updated = await (kind === 'approve'
            ...
            if (!updated)
                throw new Error('审批记录已失效，请刷新后重试。');
            setDetail(updated);
            notify('success', kind === 'approve' ? '已批准执行' : kind === 'reject' ? '已拒绝执行' : '取消请求已发送');
            signalRefresh(['runs', 'environments', 'scenarios', 'workbench']);
        }
        catch (reason) {
            setActionExplanation(actionableExplanation(reason));
            notify('error', '操作失败', displayError(reason));
        }
        finally {
    ...
    async function approveBatch() {
        if (!batchSelection.length || !batchReason.trim() || actionInFlight.current)
            return;
        actionInFlight.current = true;
        ...
        try {
            ...
            setActionExplanation(actionableExplanation(reason));
            notify('error', '批量审批失败', displayError(reason));
        }
        finally {
            actionInFlight.current = false;
            setBusy(undefined);
    ...
    async function retrySafeRange() {
        ...
        try {
            ...
            setActionExplanation(actionableExplanation(reason));
            notify('error', '无法安全续跑', displayError(reason));
        }
        finally {
            setBusy(undefined);
        }
    ...
    useEffect(() => {
        if (detail && ACTIVE.has(detail.status) && logRef.current)
    ...
    {user.role === 'platform_admin' && historyOpen && <Modal busy={Boolean(busy)} size="wide" title="运行历史管理" description="管理当前页成功记录归档、失败记录删除与自动保留策略。" onClose={() => setHistoryOpen(false)}><RunRetentionPanel runs={runs ?? []} onDeleted={handleDeletedRuns}/></Modal>}
    ...
    {loading && !runs ? <LoadingBlock label="正在加载运行队列…"/> : error && !runs ? <ErrorBlock message={error} onRetry={() => void reload()}/> : <div className="runs-layout">
      ...
      {detailLoading && !detail ? <section className="panel"><LoadingBlock label="正在加载运行详情…"/></section> : detailError ? <section className="panel"><ErrorBlock message={detailError} onRetry={() => void reloadDetail()}/></section> : detail ? <section className="run-detail detail-stack">
        <article className="panel run-hero">
          ...
          <div className="run-actions">{user.role === 'platform_admin' && detail.status === 'failed' && <Button className="button button--danger-soft" onClick={() => setCleanupIds([detail.id])} htmlType={"button"} type="default" danger><Trash2 size={14}/> 删除失败记录</Button>}{user.role === 'platform_admin' && !detail.snapshotError && detail.status === 'succeeded' && !['archived', 'queued', 'running'].includes(detail.archive?.status ?? '') ? <Button className="button button--quiet" disabled={busy === 'archive'} onClick={async () => {
                        setBusy('archive');
                        try {
                            await api.archiveRuns([detail.id]);
                            notify('success', '归档任务已受理');
                            signalRefresh('runs');
                        }
                        catch (e) {
                            notify('error', '归档失败', displayError(e));
                        }
                        finally {
                            setBusy(undefined);
                        }
                    }} htmlType={"button"} type="default">归档</Button> : null}<StatusPill status={detail.status}/>{!detail.snapshotError && ACTIVE.has(detail.status) && detail.status !== 'awaiting_approval' && <Button disabled={busy === 'cancel'} className="button button--danger-soft" onClick={() => void action('cancel')} htmlType={"button"} type="default" danger><Square size={14}/> 取消</Button>}</div>
          <div className="progress-track"><span style={{ width: `${progress}%` }}/><small>{progressLabel}</small></div>
        </article>
{detail.snapshotError && <p className="inline-warning" role="alert">{detail.snapshotError}</p>}
<RefreshNotice loading={activity.loading} error={activity.error} onRetry={activity.reload}/>
<RunWaitingPanel status={activity.data?.status ?? detail.status} observations={activity.data?.waitingObservations ?? []}/>
...
{!detail.snapshotError && detail.status === 'awaiting_approval' && <article className="panel"><header className="panel__header"><div><span className="panel__icon panel__icon--amber"><ShieldAlert size={18}/></span><div><h2>风险审批与交付决策</h2><p>{detail.approval?.riskReason ?? '该运行需要环境 Owner 审批。'}</p></div></div></header>{detail.deliveryRequirements?.length ? <div className="backup-list">{detail.deliveryRequirements.map((item) => <section key={item.id}><strong>{item.kind === 'artifact' ? '介质' : '镜像'} · {item.componentName} / {item.name}</strong><p>{item.identity}</p><small>来源：{item.source}</small><small>目标：{item.target || '环境未配置本地目标'}</small>{user.role === 'environment_owner' ? <div className="row-actions"><Radio name={item.id} checked={deliveryModes[item.id] === 'direct'} onChange={() => setDeliveryModes((current) => ({ ...current, [item.id]: 'direct' }))}> 直接使用来源</Radio><Radio name={item.id} disabled={!item.transferAvailable} checked={deliveryModes[item.id] === 'transfer'} onChange={() => setDeliveryModes((current) => ({ ...current, [item.id]: 'transfer' }))}> 平移到环境目标</Radio></div> : null}</section>)}</div> : null}{user.role === 'environment_owner' ? <div className="modal-body"><Field label={"审批理由"}><Input.TextArea rows={2} value={approvalReason} onChange={(event) => setApprovalReason(event.target.value)}/></Field><div className="row-actions"><Button disabled={Boolean(busy)} className="button button--quiet" onClick={() => void action('reject')} htmlType={"button"} type="default"><X size={15}/> 拒绝</Button><Button disabled={Boolean(busy) || (detail.deliveryRequirements ?? []).some((item) => !deliveryModes[item.id])} className="button button--primary" onClick={() => void action('approve')} htmlType={"button"} type="primary"><Check size={15}/> 批准执行</Button></div></div> : <span>仅环境 Owner 可审批</span>}</article>}
<WorkspaceTabs key={detail.id} activeKey={detailTab} onChange={setDetailTab} items={[{key: 'steps', label: '步骤与日志', children: <><RunLogDownload key={`logs-${detail.id}`} run={detail}/>
<article className="panel"><header className="panel__header"><div><span className="panel__icon"><Clock3 size={18}/></span><div><h2>执行步骤</h2><p>{detail.executionMode ? '组件验证及变更完成后，按顺序执行场景业务验收；失败立即停止并保留现场。' : '组件按顺序执行：前置检查 → 执行动作及 handler → 后置检查'}</p>{detail.jobDigest && <Button className="button button--quiet" href={api.runJobDownloadURL(detail.id)} type="default">下载本次作业包</Button>}{detail.exitCode !== undefined && <small>作业退出码：{detail.exitCode}</small>}</div></div></header>{detail.steps?.length ? <div className="step-timeline">{detail.steps.map((step, index) => <Fragment key={step.id}>{(index === 0 || `${detail.steps![index - 1].sourceNodeId}/${detail.steps![index - 1].parentActionId ?? detail.steps![index - 1].id}` !== `${step.sourceNodeId}/${step.parentActionId ?? step.id}`) && <div className="step-group-heading"><strong>{step.componentName || step.name || scenarioStepLabel(step)}{step.parentAction ? ` · ${{ install: '安装', rollback: '回滚', configure: '配置', upgrade: '升级', uninstall: '卸载' }[step.parentAction] ?? step.parentAction}` : ''}</strong>{step.sourceNodeId && <small>节点 {step.sourceNodeId}</small>}</div>}<div id={`run-step-${step.id}`} key={step.id} className={`step step--${step.status}`}><span className="step__index">{step.status === 'succeeded' ? <Check size={14}/> : step.status === 'failed' ? <X size={14}/> : index + 1}</span><span className="step__line"/><div><div><strong>{scenarioStepLabel(step)}</strong><StatusPill status={step.status}/></div><p>{step.hostGroup && <><HostGroupName value={step.hostGroup}/> · </>}{step.componentName ? `${step.componentName} · ` : ''}{step.action ?? ''} · {scenarioStepLabel(step)}{step.summary ? ` · ${step.summary}` : ''}</p><small>{formatTime(step.startedAt)}{step.finishedAt ? ` → ${formatTime(step.finishedAt)}` : ''}</small></div></div></Fragment>)}</div> : <EmptyState title={detail.snapshotError ? '未记录执行步骤' : '步骤尚未生成'} description={detail.snapshotError ? '请查看诊断和日志中的故障信息。' : detail.status === 'awaiting_approval' ? '审批通过后进入环境队列。' : 'Planner 正在生成执行步骤。'}/>}</article>
{detail.archive?.status === 'archived' ? <article className="panel"><header className="panel__header"><div><span className="panel__icon panel__icon--cyan"><Archive size={18}/></span><div><h2>历史日志已归档</h2><p>完整脱敏日志保存在归档包内。{formatFileSize(detail.archive.sizeBytes)}</p></div></div><Button className="button button--quiet" href={api.archiveDownloadURL(detail.id)} type="default"><Download size={14}/> 下载归档包</Button></header></article> : <article className="panel log-panel"><header className="panel__header"><div><span className="panel__icon panel__icon--cyan"><ScrollText size={18}/></span><div><h2>{ACTIVE.has(detail.status) ? '实时日志' : '历史日志'}</h2><p>最近最多 2000 条 · 凭据自动脱敏 · 完整日志可下载</p></div></div><span className={`live-badge${ACTIVE.has(detail.status) ? '' : ' live-badge--history'}`}><span /> {ACTIVE.has(detail.status) ? 'LIVE' : 'ARCHIVED'}</span></header><div className="log-toolbar"><label><Search size={14}/><Input aria-label="搜索运行日志" value={logQuery} onChange={(event) => setLogQuery(event.target.value)} placeholder="搜索主机、任务或错误"/></label><Select aria-label="日志流筛选" value={logStream} onChange={(selectedValue) => setLogStream(selectedValue as typeof logStream)} popupMatchSelectWidth={true}><Select.Option value="all">全部流</Select.Option><Select.Option value="stdout">stdout</Select.Option><Select.Option value="stderr">stderr</Select.Option><Select.Option value="system">system</Select.Option></Select><span>{visibleLogLines.length}/{allLogLines.length} 行</span><Button className="icon-text" onClick={() => void navigator.clipboard?.writeText(visibleLogLines.join('\n'))} htmlType={"button"} type="text"><Copy size={14}/> 复制结果</Button><Button className="icon-text" onClick={downloadLogs} htmlType={"button"} type="text"><Download size={14}/> 下载已加载日志</Button></div><pre ref={logRef}>{allLogLines.length ? visibleLogLines.join('\n') || '没有匹配的日志行。' : `[platform] Run ${detail.id}\n[platform] status=${detail.status}\n[platform] 等待 Ansible 输出…`}</pre></article>}</>},
{key: 'delivery', label: '输入与交付', children: <><RunDeliveryPanel run={detail}/>
...
{key: 'diagnostics', label: '诊断', children: <><RunDiagnosticsPanel key={detail.id} run={detail} retryBusy={busy === 'retry'} onRetryRun={() => void retrySafeRange()} onLocateStep={stepId => { setDetailTab('steps'); setStepToLocate(stepId); }}/></>}]} />

      </section> : <section className="panel"><EmptyState title="选择一个运行" description="查看步骤、审批和日志详情。"/></section>}
    </div>}
    {batchOpen && <Modal busy={Boolean(busy)} title={`批量批准 ${batchSelection.length} 个危险运行`} description={`共 ${batchCandidates.length} 个候选；每批最多 100 条。仅包含无需逐项交付选择的 Run，服务端原子校验本次勾选的审批。`} onClose={() => setBatchOpen(false)} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={() => setBatchOpen(false)} htmlType={"button"} type="default">取消</Button><Button className="button button--primary" disabled={busy === 'batch-approve' || !batchReason.trim() || !batchSelection.length} onClick={() => void approveBatch()} htmlType={"button"} type="primary"><Check size={15}/> {busy === 'batch-approve' ? '批准中…' : '确认批量批准'}</Button></footer>}><div className="modal-body"><div className="batch-approval-list">{batchCandidates.map((run) => { const checked = batchSelection.some((item) => item.id === run.id); return <Checkbox key={run.id} aria-label={`审批候选 ${run.id}`} checked={checked} disabled={busy === 'batch-approve' || (!checked && batchSelection.length >= 100)} onChange={() => setBatchSelection((current) => checked ? current.filter((item) => item.id !== run.id) : [...current, run])}><strong>{run.name}</strong><span>{run.environmentName} · Run {run.id} · 待风险审批</span></Checkbox>; })}</div><Field label={"审批理由"}><Input.TextArea rows={3} value={batchReason} onChange={(event) => setBatchReason(event.target.value)}/></Field></div></Modal>}
  </div>;
}
