...
function sections(workbench: Workbench): Section[] {
  ...
  return [
    ...
    { title: '等待我处理', description: 'Draft、证据、测试和环境维护事项', icon: AlertTriangle, items: action, tone: 'action' },
...
export function DashboardPage() {
  ...
  return <div className="page page--dashboard page--workbench">
    <PageHeader
      ...
      actions={<div className="workbench-header-actions"><small>生成于 {formatTime(workbench.generatedAt)}</small>{total ? <Link className="button button--primary" to={workbench.items[0].primaryAction.href}><PlayCircle size={16} /> 处理首要事项</Link> : null}</div>}
    />
    ...
    {!total ? <section className="panel workbench-empty"><CheckCircle2 size={34} /><EmptyState title="当前没有待办" description="你负责的交付对象没有阻断、活动运行或待评估影响。" /></section> : <div className="work-sections">
      {grouped.map((section) => section.items.length ? <section className={`panel work-section work-section--${section.tone}`} key={section.title}>
        ...
        <div className="work-list">{section.items.map((item) => <WorkCard item={item} key={item.id} />)}</div>
      </section> : null)}
    </div>}

    <section className="panel work-assets">
