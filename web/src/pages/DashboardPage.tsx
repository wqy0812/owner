...
function subjectLabel(item: WorkItem) {
    if (item.subject.version)
        return item.subject.version;
    if (item.subject.revision)
        return `版本 ${item.subject.revision}`;
    if (item.subject.environment)
        return item.subject.environment;
    return item.subject.type.replaceAll('_', ' ');
}
function WorkCard({ item, onReview }: {
    item: WorkItem;
    ...
    <div className="work-card__body">
      ...
      <details className="work-card__details"><summary>查看 {item.reasons.length} 项原因与建议</summary><ul>{item.reasons.map((reason) => <li key={`${reason.code}-${reason.evidenceRunId ?? reason.message}`}><span /><div><strong>{reason.message}</strong>{reason.cause && <small>{reason.cause.actorName ? `${reason.cause.actorName} · ` : ''}{reason.cause.summary}{reason.cause.at ? ` · ${formatTime(reason.cause.at)}` : ''}</small>}<div>{reason.evidenceRunId && reason.nextAction?.href !== `/runs?selected=${reason.evidenceRunId}` && <Link to={`/runs?selected=${reason.evidenceRunId}`}>查看证据 Run</Link>}{reason.nextAction && <Link to={reason.nextAction.href}>{reason.nextAction.label}</Link>}</div></div></li>)}</ul></details>
    </div>
    <div className="work-card__actions">{item.kind === 'component_review' && onReview ? <Button className="button button--primary" onClick={() => onReview(item.subject.id)} htmlType={"button"} type="primary">{item.primaryAction.label}<ArrowRight size={14}/></Button> : <RouteButton className="button button--primary" to={item.primaryAction.href} type="primary">{item.primaryAction.label}<ArrowRight size={14}/></RouteButton>}{item.secondaryActions.map((action) => <RouteButton key={action.href} className="icon-text" to={action.href} type="text">{action.label}</RouteButton>)}</div>
  </article>;
}
export function DashboardPage() {
    ...
    if (!query.data && !query.error)
        return <LoadingBlock label="正在整理我的交付待办…"/>;
    if (query.error && !query.data)
        return <ErrorBlock message={query.error} onRetry={() => void query.reload()}/>;
    ...
    <PageHeader eyebrow="My delivery work" title={`早上好，${user.name.split('·')[0].trim()}`} description={`当前身份：${ROLE_LABELS[user.role]}。这里按优先级告诉你为什么被阻塞，以及下一步前往哪里处理。`} actions={<div className="workbench-header-actions"><small>生成于 {formatTime(workbench.generatedAt)}</small>{total ? workbench.items[0].kind === 'component_review' ? <Button className="button button--primary" onClick={() => setReviewReleaseId(workbench.items[0].subject.id)} htmlType={"button"} type="primary"><PlayCircle size={16}/> 处理首要事项</Button> : <RouteButton className="button button--primary" to={workbench.items[0].primaryAction.href} type="primary"><PlayCircle size={16}/> 处理首要事项</RouteButton> : null}</div>}/>
