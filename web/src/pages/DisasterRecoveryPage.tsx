...
function CatalogRepositoryPanel() {
    ...
    <header className="panel__header catalog-repository-panel__header">
      ...
      <div className="catalog-repository-panel__toolbar" role="group" aria-label="灾备操作">
        <Button className={`button ${repository?.configured || !repository?.targetCatalogEmpty ? 'button--quiet' : 'button--danger'}`} disabled={!repositoryEnabled || backupBusy} title={repository && !repository.enabled ? repository.reason : undefined} onClick={() => setRepositoryMode(repository?.configured || !repository?.targetCatalogEmpty ? 'connect' : 'restore')} htmlType={"button"}>{repository?.configured ? '更换备份仓库' : repository?.targetCatalogEmpty ? '从已有 Git 仓库恢复' : '接入已有备份仓库'}</Button>
        <Button className="button button--secondary" disabled={!repositoryEnabled || backupBusy} title={repository && !repository.enabled ? repository.reason : undefined} onClick={() => setRepositoryMode('create')} htmlType={"button"} type="default"><Plus size={15}/> 创建私有仓库</Button>
        <Button className="button button--primary" disabled={!repositoryEnabled || !repository?.configured || backupBusy} title={!repositoryEnabled ? repository?.reason : !repository?.configured ? '请先创建或接入私有仓库' : undefined} onClick={() => void backupNow()} htmlType={"button"} type="primary">{backupBusy ? '正在备份…' : <><Save size={15}/> 立即备份</>}</Button>
      </div>
    </header>
...
function CatalogRepositoryModal({ mode, allowedRoot, onClose, onDone }: {
    mode: 'create' | 'connect' | 'restore';
    allowedRoot: string;
    onClose: () => void;
    onDone: (status: CatalogRepositoryStatus, message: string, continueToRestore?: boolean) => void;
}) {
    const { notify } = useApp();
    ...
    async function submit(continueToRestore = false) {
        ...
        try {
            ...
            notify('error', mode === 'create' ? '创建私有仓库失败' : '接入私有仓库失败', displayError(reason));
        }
        finally {
            setBusy(false);
        }
    ...
    return <Modal busy={Boolean(busy)} title={mode === 'create' ? '创建私有 Catalog 仓库' : restoreMode ? '从已有 Git 仓库恢复' : '接入已有 Catalog 仓库'} description={restoreMode ? `先验证并接入 ${allowedRoot} 内的仓库，再选择恢复点并预检。` : `路径必须位于 ${allowedRoot} 内；相对路径会基于该目录解析。`} onClose={onClose} formProps={{ onFinish: () => submit(restoreMode), layout: "vertical", preserve: false }} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={busy} onClick={onClose} htmlType={"button"} type="default">取消</Button>{restoreMode && <Button className="button button--quiet" disabled={busy || !path.trim() || Boolean(pathState.error)} onClick={() => void submit(false)} htmlType={"button"} type="default">仅接入，暂不恢复</Button>}<Button className="button button--primary" disabled={busy || !path.trim() || Boolean(pathState.error)} htmlType={"submit"} type="primary">{busy ? '处理中…' : mode === 'create' ? '创建并接入' : restoreMode ? '验证并继续恢复' : '验证并接入'}</Button></footer>}><div className="modal-body"><Field label={"服务器仓库路径"} extra={<><small>{mode === 'create' ? '目标必须不存在；平台将创建权限为 0700 的 bare Git 仓库。' : restoreMode ? '已有仓库必须包含 catalog 分支；验证成功后继续选择远端 backup/* 恢复点。' : `已有仓库必须包含 catalog 分支，且路径位于 ${allowedRoot}。`}</small>{pathState.resolvedPath && !pathState.error ? <small>最终路径：<code>{pathState.resolvedPath}</code></small> : null}</>} required><Input aria-label="服务器仓库路径" aria-invalid={Boolean(pathState.error)} value={path} onChange={(event) => setPath(event.target.value)} placeholder={mode === 'create' ? 'catalog-production.git' : '/data/private-catalog-repositories/catalog-production.git'} autoFocus required/></Field>{pathState.error ? <div className="form-validation" role="alert">{pathState.error}</div> : null}</div></Modal>;
}
function catalogRepositoryPathState(allowedRoot: string, input: string): {
    resolvedPath?: string;
    error?: string;
} {
...
function CatalogRestoreModal({ repository, onClose, onDone }: {
    repository: CatalogRepositoryStatus;
    onClose: () => void;
    onDone: () => void;
}) {
    const { notify } = useApp();
    ...
    async function preview() {
        ...
        setConfirmation('');
        try {
            setPlan(await api.previewCatalogRestore(ref));
        }
        catch (reason) {
            notify('error', '恢复预检失败', displayError(reason));
        }
        finally {
            setBusy(undefined);
        }
    ...
    async function restoreCatalog() {
        ...
        setBusy('restore');
        try {
            await api.restoreCatalog({ ref, expectedPlanDigest: plan.planDigest, confirmation });
            onDone();
        }
        catch (reason) {
            setPlan(undefined);
            notify('error', 'Git 恢复失败', displayError(reason));
        }
        finally {
            setBusy(undefined);
        }
    }
    return <Modal busy={Boolean(busy)} size="wide" title="从 Git 恢复发布目录" description="仅在当前数据库没有任何组件和场景时允许恢复；操作不会恢复 Run、环境、审批或审计历史。" onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={Boolean(busy)} onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--quiet" disabled={Boolean(busy) || !ref} onClick={() => void preview()} htmlType={"button"} type="default">{busy === 'preview' ? '预检中…' : '预览恢复'}</Button><Button className="button button--danger" disabled={Boolean(busy) || !plan || confirmation !== '恢复发布目录'} onClick={() => void restoreCatalog()} htmlType={"button"} type="default" danger>{busy === 'restore' ? '恢复中…' : '确认恢复空库'}</Button></footer>}>
    <div className="modal-body cluster-rollback-preview"><div className="warning-callout"><AlertTriangle size={19}/><div><strong>冲突时整批失败</strong><p>组件或场景非空、ID/slug 冲突、用户身份属性冲突、Playbook 路径内容不同，都会拒绝恢复且不留下部分数据。</p></div></div>
      <Field label={"Git 恢复点"}><Select aria-label="Git 恢复点" value={ref} onChange={(selectedValue) => { setRef(selectedValue); setPlan(undefined); setConfirmation(''); }} popupMatchSelectWidth={true}>{repository.recoveryPoints.map((point) => <Select.Option key={point.ref} value={point.ref}>{formatTime(point.createdAt)} · {point.ref}</Select.Option>)}</Select></Field>
      {plan && <><section className="cluster-rollback-summary"><div><span>组件</span><strong>{plan.counts.components ?? 0}</strong></div><div><span>Release</span><strong>{plan.counts.component_releases ?? 0}</strong></div><div><span>场景</span><strong>{plan.counts.scenarios ?? 0}</strong></div><div><span>Playbook</span><strong>{plan.playbookCount}</strong></div></section><div className="revision-diff"><GitCompare size={18}/><div><strong>恢复计划已锁定</strong><p>Commit：{plan.gitCommit.slice(0, 16)}…</p><p>Catalog：{plan.catalogSha256.slice(0, 16)}… · Schema：{plan.schemaContract}</p></div></div><Field label={"输入“恢复发布目录”以确认"} extra={<><small>恢复前会重新核对空库状态和计划摘要。</small></>}><Input aria-label="确认恢复发布目录" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} autoComplete="off"/></Field></>}
    </div>
  </Modal>;
}
