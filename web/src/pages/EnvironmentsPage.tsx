...
import { Activity, AlertTriangle, Archive, ArchiveRestore, Braces, CheckCircle2, CloudCog, Cpu, Download, GitCompare, HardDrive, History, KeyRound, LockKeyhole, Network, Plus, RotateCcw, Save, Server, Trash2, UserRound, Wifi } from 'lucide-react';
...
import { EnvironmentRevisionDeletionModal } from '../components/EnvironmentRevisionDeletionModal';
...
function CredentialSourceList({ sources, name, loading }: {
    sources?: CredentialSource[];
    name: string;
    loading: boolean;
}) {
    if (!sources)
        return loading ? <small>正在读取声明来源…</small> : null;
...
function revisionActor(revision: EnvironmentRevision, users: Array<{
    id: string;
    name: string;
}>): string {
...
export function EnvironmentsPage() {
    ...
    const [deleteRevision, setDeleteRevision] = useState<EnvironmentRevision>();
    ...
    useEffect(() => { setDeleteRevision(undefined); }, [selected?.id]);
    useEffect(() => {
    ...
    // Refreshing checks after a history deletion must not discard unsaved edits
    // to the unchanged current version in another tab.
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (requested === 'inventory' || requested === 'facts' || requested === 'parameters' || requested === 'variables' || requested === 'credentials')
            setTab(requested);
        const focus = searchParams.get('focus');
        if (focus)
            requestAnimationFrame(() => document.getElementById(focus === 'health' ? 'environment-health' : `environment-variable-${focus}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }));
    ...
    function discardCurrent() {
        if (!baseline)
            return;
        if (tab === 'inventory')
            setHosts(baseline.hosts);
        if (tab === 'facts')
            setFacts(baseline.facts);
        if (tab === 'parameters')
            setParameters(baseline.parameters);
        if (tab === 'variables')
            setVariables(Object.entries(baseline.variables).map(([name, value]) => ({ name, value })));
        if (tab === 'credentials')
            setCredentials(baseline.credentialRefs);
    }
    async function saveCurrent(changeReason: string) {
        if (!selected || !editable || !dirty)
            return;
        setBusy(true);
        try {
            if (tab === 'inventory')
                await api.updateInventory(selected.id, hosts, changeReason);
            if (tab === 'facts')
                await api.updateFacts(selected.id, facts, changeReason);
            if (tab === 'parameters') {
            ...
            if (tab === 'variables') {
                ...
                if (names.some((name) => !/^[A-Z_][A-Z0-9_]*$/.test(name)))
                    throw new Error('环境变量名必须是大写标识符。');
                if (new Set(names).size !== names.length)
                    throw new Error('环境变量名不能重复。');
                const governed = new Set((variableDefinitions ?? []).map((item) => item.name));
                if (names.some((name) => !governed.has(name)))
                    throw new Error('环境变量必须从平台 Owner 维护的字段中选择。');
                await api.updateVariables(selected.id, variablesObject, changeReason);
            }
            if (tab === 'credentials')
                await api.updateCredentialRefs(selected.id, credentials.map(({ name, type, reference }) => ({ name, type, reference })), changeReason);
            setSaveOpen(false);
            notify('success', '环境版本已更新', '差异和变更原因已记录，后续运行会锁定新快照。');
    ...
    async function restore(reason: string) {
        if (!selected || !restoreRevision)
            return;
        setBusy(true);
        try {
            await api.restoreEnvironmentRevision(selected.id, restoreRevision.id, reason);
            notify('success', `已基于 r${restoreRevision.revision} 创建新版本`, '历史没有被覆盖，后续 Run 将锁定恢复后的新快照。');
            setRestoreRevision(undefined);
            signalRefresh(['environments', 'workbench']);
        }
        catch (errorReason) {
            notify('error', '版本恢复失败', displayError(errorReason));
        }
        finally {
            setBusy(false);
        }
    ...
    async function exportRevision(revision: EnvironmentRevision, includeCredentialReferences: boolean) {
        if (!selected || exportBusy)
            return;
        if (includeCredentialReferences && !await confirm('敏感导出会包含 CredentialRef 的引用字符串，但不会包含真实 Secret。确认下载并自行妥善保管？'))
            return;
        setExportBusy(true);
        try {
            ...
            anchor.href = url;
            anchor.download = `${selected.name}-r${revision.revision}${includeCredentialReferences ? '-credential-refs' : ''}.json`;
            anchor.click();
            URL.revokeObjectURL(url);
            notify('success', includeCredentialReferences ? '已导出含凭据引用的环境文件' : '已安全导出环境文件', '导出内容不包含 Secret 实际值、运行记录或历史证据。');
        }
        catch (reason) {
            notify('error', '环境导出失败', displayError(reason));
        }
        finally {
            setExportBusy(false);
        }
    ...
    async function submitClusterRollback(confirmEnvironmentName: string) {
        if (!selected || !rollbackPlan || rollbackPlan.componentCount === 0 || !rollbackPlan.planDigest)
            return;
    ...
    async function openLifecycle() {
        if (!selected || !ownsSelected || anyDirty)
            return;
    ...
    async function applyLifecycle(action: 'delete' | 'archive' | 'unarchive') {
        ...
        try {
            if (action === 'delete') {
                ...
                notify('success', '环境已删除', '从未使用的环境及其版本、健康检查已永久删除，审计记录已保留。');
                setSearchParams({});
            }
            else if (action === 'archive') {
                await api.archiveEnvironment(selected.id);
                notify('success', '环境已归档', '历史 Run、版本和构建证据仍可追溯；该环境不再参与新任务选择。');
            }
            else {
            ...
            catch { /* Retain the actionable mutation error. */ }
        }
        finally {
            setLifecycleBusy(undefined);
        }
    ...
    const revisionActionBusy = busy || exportBusy || healthBusy || rollbackBusy !== undefined || lifecycleBusy !== undefined || Boolean(deleteRevision);
    return <div className="page">
    <PageHeader eyebrow="Execution environments" title="环境管理" description="分别查看调度占用与真实连通性；配置变更以可追溯版本保存。" actions={user.role === 'environment_owner' ? <><ActionMenu ariaLabel="环境目录更多操作" items={[{ key: "import", label: "导入环境", onClick: () => setImportOpen(true) }]}/><Button className="button button--primary" onClick={() => setCreateOpen(true)} htmlType={"button"} type="primary"><Plus size={16}/> 新建环境</Button></> : undefined}/>
      ...
      {selected ? <section className="detail-stack">
        <article className="panel environment-hero">
          ...
          {(ownsSelected || (editable && selected.currentRevision)) && <div className="environment-hero__toolbar" role="group" aria-label="环境操作">
            <ActionMenu items={[
              editable && selected.currentRevision && { key: 'export', label: '安全导出', disabled: revisionActionBusy, onClick: () => exportRevision(selected.currentRevision!, false) },
              editable && selected.currentRevision && { key: 'export-credentials', label: '导出含凭据', disabled: revisionActionBusy, onClick: () => exportRevision(selected.currentRevision!, true) },
              ownsSelected && { key: 'lifecycle', label: selected.archivedAt ? '恢复环境' : '移除环境', danger: !selected.archivedAt, disabled: anyDirty || lifecycleBusy !== undefined, onClick: openLifecycle },
              editable && selected.currentRevision && { key: 'reset', label: '重置环境', danger: true, disabled: (selected.schedulingStatus ?? 'idle') !== 'idle' || anyDirty || rollbackBusy !== undefined, onClick: previewClusterRollback },
            ]}/>
          </div>}
        </article>
...
<WorkspaceTabs key={selected.id} items={[{key: 'configuration', label: '配置', children: <><section className="fact-grid">
...
<article className="panel health-panel" id="environment-health">
          <header className="panel__header"><div><span className={`panel__icon ${health?.status === 'degraded' || sshCheck?.status === 'degraded' ? 'panel__icon--rose' : 'panel__icon--cyan'}`}><Activity size={18}/></span><div><h2>环境连通性</h2><p>一次完成 TCP 端点探测和 Go SSH 检查；检查只读，不执行安装。</p></div></div>{editable && <Button className="button button--secondary" disabled={healthBusy} onClick={() => void checkHealth()} htmlType={"button"} type="default"><Wifi size={15}/> {healthBusy ? '正在检查 TCP 与 SSH…' : '立即检查'}</Button>}</header>
          ...
          {!health && !sshCheck ? <EmptyState title="尚未检查" description="“调度空闲”不代表端口可达、SSH 可认证或远端会话可执行。"/> : <div className="connectivity-results">
            ...
            <section className="health-results" aria-label="SSH 检查结果">
              ...
              {!sshCheck ? <EmptyState title="SSH 尚未检查"/> : <>
                <div className={`health-summary health-summary--${sshCheck.status}`}>{sshCheck.status === 'healthy' ? <CheckCircle2 size={18}/> : <AlertTriangle size={18}/>}<div><strong>{sshCheck.status === 'healthy' ? 'SSH 检查通过' : 'SSH 检查存在异常'}</strong><small>{formatTime(sshCheck.checkedAt)} · {sshCheck.results.filter((item) => item.status === 'passed' || item.status === 'skipped').length}/{sshCheck.results.length} 通过 · {sshCheck.durationMs} ms{sshCheckStale ? ' · 基于旧版本，请重新检查' : ''}</small></div></div>
...
<article className="panel environment-editor">
 ...
 ['inventory', '主机分组', <Server size={16}/>], ['facts', '环境标签', <Cpu size={16}/>], ['parameters', '组件环境参数', <CloudCog size={16}/>], ['variables', '环境变量', <Braces size={16}/>], ['credentials', '凭据引用', <KeyRound size={16}/>],
          ...
          <div className="persistent-section" hidden={!(tab === 'facts')}>{<div className="editor-section"><div className="section-title"><div><h3>环境标签</h3><p>每个分类选择一个实际值；子分类须与所选父分类对应。</p></div></div><EnvironmentFactFields categories={platformOptionCategories} facts={facts} editable={editable} onChange={setFacts}/></div>}</div>
          <div className="persistent-section" hidden={!(tab === 'parameters')}>{<div className="editor-section"><div className="section-title"><div><h3>组件环境参数</h3><p>字段由组件版本定义，环境 Owner 填写实际值或采用组件建议值。保存后记录到环境版本。</p></div></div>
            <div className="scenario-parameter-fields">{(parameterFields ?? []).map((field) => {
                ...
                <ParameterValueEditor parameter={{ name: field.label, type: field.type, enum: field.enum }} value={value ?? undefined} disabled={!editable} optional={!required} onChange={(nextValue) => setParameters((current) => {
                                const next = { ...current };
                                if (nextValue === undefined)
                                    delete next[field.valueKey];
                                else
                                    next[field.valueKey] = nextValue ?? null;
                                return next;
                            })}/>
                {editable && value === undefined && field.suggestedValue !== undefined ? <Button className="icon-text" onClick={() => setParameters((current) => ({ ...current, [field.valueKey]: field.suggestedValue }))} htmlType={"button"} type="text">采用建议值 {String(field.suggestedValue)}</Button> : null}
              </label>;
            ...
            {staleParameterKeys.length > 0 && <section className="stale-parameter-notice" aria-label="失效环境参数">
              <header><span className="stale-parameter-notice__icon"><AlertTriangle size={18}/></span><div><div className="stale-parameter-notice__title"><h3>已从组件契约移除的参数</h3><em>{staleParameterKeys.length} 项待清理</em></div><p>当前契约不再引用这些值。移除全部项目后保存为新版本，历史版本仍会完整保留。</p></div></header>
              <div className="stale-parameter-list">{staleParameterKeys.map((key) => <article key={key} className="stale-parameter-item"><div><strong>{key.split(':').at(-1)}</strong><small>当前环境保留值</small></div><code>{JSON.stringify(effectiveParameters[key])}</code>{editable && <Button className="button button--danger-soft" aria-label={`移除失效参数 ${key}`} onClick={() => setParameters((current) => { const next = { ...current }; delete next[key]; return next; })} htmlType={"button"} type="default" danger><Trash2 size={14}/> 移除</Button>}</article>)}</div>
            </section>}
          </div>}</div>
          <div className="persistent-section" hidden={!(tab === 'variables')}>{<div className="editor-section"><div className="section-title"><div><h3>组件作业环境变量</h3><p>变量键由平台 Owner 治理，环境 Owner 只选择字段并填写非敏感值。</p></div>{editable && <Button className="button button--quiet" disabled={(variableDefinitions ?? []).every((definition) => variables.some((item) => item.name === definition.name))} onClick={() => setVariables((items) => [...items, { name: '', value: '' }])} htmlType={"button"} type="default"><Plus size={15}/> 添加变量</Button>}</div><div className="environment-variable-list">{variables.map((variable, index) => <div key={index} id={variable.name ? `environment-variable-${variable.name}` : undefined}><span className="variable-icon"><Braces size={17}/></span><Select aria-label="环境变量名" value={variable.name} disabled={!editable} onChange={(selectedValue) => setVariables((items) => items.map((item, i) => i === index ? { ...item, name: selectedValue } : item))} popupMatchSelectWidth={true}><Select.Option value="">请选择平台字段</Select.Option>{variable.name && !(variableDefinitions ?? []).some((definition) => definition.name === variable.name) ? <Select.Option value={variable.name}>{variable.name} · 历史字段</Select.Option> : null}{(variableDefinitions ?? []).filter((definition) => definition.name === variable.name || !variables.some((item) => item.name === definition.name)).map((definition) => <Select.Option key={definition.id} value={definition.name}>{definition.label} · {definition.name}</Select.Option>)}</Select><Input aria-label={`环境变量 ${variable.name || index} 的值`} value={variable.value} disabled={!editable} placeholder="非敏感字符串值" onChange={(event) => setVariables((items) => items.map((item, i) => i === index ? { ...item, value: event.target.value } : item))}/>{editable && <Button className="icon-button icon-button--danger" aria-label={`移除环境变量 ${variable.name || index}，保存后生效`} onClick={() => setVariables((items) => items.filter((_, i) => i !== index))} htmlType={"button"} type="text" danger><Trash2 size={15}/></Button>}</div>)}</div>{!variables.length && <EmptyState title="尚未配置环境变量" description={editable ? '从平台字段目录选择变量。' : '环境 Owner 尚未配置环境变量。'}/>}</div>}</div>
          <div className="persistent-section" hidden={!(tab === 'credentials')}>{<div className="editor-section"><div className="section-title"><div><h3>CredentialRef</h3><p>查看凭据的声明来源，填写后端变量名或密钥文件路径。声明来源不表示已在本环境运行。</p></div>{editable && <Button className="button button--quiet" onClick={() => setCredentials((items) => [...items, { name: '', type: 'envVarRef', reference: '' }])} htmlType={"button"} type="default"><Plus size={15}/> 添加引用</Button>}</div>{credentialSources.error && <ErrorBlock message={credentialSources.error} onRetry={() => void credentialSources.reload()}/>}<div className="credential-list">{credentials.map((credential, index) => <div className="credential-entry" key={index}><div className="credential-fields"><span className="credential-icon"><LockKeyhole size={17}/></span><Input aria-label="凭据名称" value={credential.name} disabled={!editable} onChange={(event) => setCredentials((items) => items.map((item, i) => i === index ? { ...item, name: event.target.value } : item))}/><Select aria-label={`凭据 ${credential.name || index} 类型`} value={credential.type} disabled={!editable} onChange={(selectedValue) => setCredentials((items) => items.map((item, i) => i === index ? { ...item, type: selectedValue as CredentialRef['type'] } : item))} popupMatchSelectWidth={true}><Select.Option value="envVarRef">后端环境变量引用</Select.Option><Select.Option value="sshKeyPath">后端密钥文件路径</Select.Option></Select><Input aria-label={`凭据 ${credential.name || index} 引用`} value={editable ? credential.reference ?? '' : credential.maskedReference ?? '••••••••'} disabled={!editable} placeholder={credential.type === 'envVarRef' ? 'SECRET_ENV_VAR' : '/path/to/key'} onChange={(event) => setCredentials((items) => items.map((item, i) => i === index ? { ...item, reference: event.target.value } : item))}/>{editable && <Button className="icon-button icon-button--danger" aria-label={`移除凭据 ${credential.name || index}，保存后生效`} onClick={() => setCredentials((items) => items.filter((_, i) => i !== index))} htmlType={"button"} type="text" danger><Trash2 size={15}/></Button>}</div><small>当前值：{selected.name} · r{selected.currentRevision?.revision} · {(editable ? credential.reference : credential.configured) ? '已填写引用' : '未填写引用'}</small><CredentialSourceList sources={credentialSources.data} name={credential.name} loading={credentialSources.loading}/></div>)}</div></div>}</div>
          {editable && <footer className="editor-footer"><span className={dirty ? 'editor-dirty' : ''}><LockKeyhole size={14}/> {dirty ? '当前页有未保存更改' : '当前页与已保存版本一致'}</span><div>{dirty && <Button className="button button--quiet" onClick={discardCurrent} htmlType={"button"} type="default">放弃本页更改</Button>}<Button className="button button--primary" disabled={busy || !dirty || (tab === 'parameters' && (parameterFields === undefined || staleParameterKeys.length > 0))} onClick={() => setSaveOpen(true)} htmlType={"button"} type="primary"><Save size={16}/> 保存新版本</Button></div></footer>}
        </article></>},
{key: 'history', label: '版本历史', children: <><article className="panel revision-history">
          <header className="panel__header"><div><span className="panel__icon"><History size={18}/></span><div><h2>版本历史</h2><p>恢复旧配置时始终创建新版本；非当前且未被 Run 或镜像构建引用的版本可删除。</p></div></div></header>
          <div className="revision-list">{(selected.revisions ?? (selected.currentRevision ? [selected.currentRevision] : [])).map((revision) => <div key={revision.id}>
            <span className="revision-number">r{revision.revision}</span>
            <div><strong>{revision.changeReason || (revision.revision === 1 ? '创建环境' : '未填写变更原因')}</strong><small>{revisionActor(revision, users)} · {formatTime(revision.createdAt)}</small></div>
            <div className="revision-actions">
              {revision.id === selected.currentRevision?.id ? <StatusPill status="active">当前</StatusPill> : editable && <>
                <Button className="button button--quiet" disabled={anyDirty || revisionActionBusy} onClick={() => setRestoreRevision(revision)} htmlType={"button"} type="default"><RotateCcw size={14}/> 基于此恢复</Button>
                <Button className="button button--danger-soft" aria-label={`删除版本 r${revision.revision}`} disabled={anyDirty || revisionActionBusy} title={anyDirty ? '请先保存或放弃环境配置更改' : undefined} onClick={() => setDeleteRevision(revision)} htmlType={"button"} type="default" danger><Trash2 size={14}/> 删除版本</Button>
              </>}
              {editable && <Button className="button button--quiet" disabled={revisionActionBusy} onClick={() => void exportRevision(revision, false)} htmlType={"button"} type="default"><Download size={14}/> 安全导出</Button>}
            </div>
          </div>)}</div>
        </article></>},
{key: 'runs', label: '最近运行', children: <><article className="panel"><header className="panel__header"><div><span className="panel__icon panel__icon--amber"><LockKeyhole size={18}/></span><div><h2>最近环境运行</h2><p>同一环境一次只允许一个活动 Run</p></div></div></header><RefreshNotice loading={recentRunQuery.isRefreshing} error={recentRunQuery.error} onRetry={() => void recentRunQuery.reload()}/>{environmentRuns.length ? <div className="simple-table">{environmentRuns.map((run) => <Link key={run.id} to={`/runs?selected=${run.id}`}><div><strong>{run.name}</strong><small>{formatTime(run.createdAt)}</small></div><StatusPill status={run.status}/></Link>)}</div> : recentRunQuery.loading ? <LoadingBlock label="正在读取最近运行…"/> : !recentRunQuery.error ? <EmptyState title="暂无运行记录"/> : null}</article></>}]} />
    ...
    {deleteRevision && selected && <EnvironmentRevisionDeletionModal key={`${selected.id}:${deleteRevision.id}`} environment={selected} revision={deleteRevision} onClose={() => setDeleteRevision(undefined)} onDeleted={() => {
                notify('success', `已删除环境版本 r${deleteRevision.revision}`, '该版本及其连通性检查已删除，审计记录保留。');
    ...
    {rollbackOpen && selected && <ClusterRollbackModal environment={selected} plan={rollbackPlan} error={rollbackError} explanation={rollbackExplanation} busy={rollbackBusy} onRetry={() => void previewClusterRollback()} onClose={() => {
                if (!rollbackBusy) {
                    setRollbackOpen(false);
                    setRollbackPlan(undefined);
                    setRollbackError(undefined);
            ...
            }} onConfirm={(confirmation) => void submitClusterRollback(confirmation)}/>}
    {lifecycleOpen && selected && <EnvironmentLifecycleModal environment={selected} lifecycle={lifecycle} error={lifecycleError} explanation={lifecycleExplanation} busy={lifecycleBusy} onRetry={() => void openLifecycle()} onClose={() => {
                if (!lifecycleBusy)
                    setLifecycleOpen(false);
            }} onConfirm={(action) => void applyLifecycle(action)}/>}
  </div>;
}
function EnvironmentLifecycleModal({ environment, lifecycle, error, explanation, busy, onRetry, onClose, onConfirm }: {
    environment: Environment;
    lifecycle?: EnvironmentLifecycle;
    error?: string;
    explanation?: WorkExplanation;
    busy?: 'load' | 'delete' | 'archive' | 'unarchive';
    onRetry: () => void;
    onClose: () => void;
    onConfirm: (action: 'delete' | 'archive' | 'unarchive') => void;
}) {
    ...
    return <Modal size="wide" title={title} description={`目标环境：${environment.name}`} onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={busy !== undefined} onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className={action === 'delete' ? 'button button--danger' : 'button button--primary'} disabled={blocked || !confirmed || busy !== undefined} onClick={() => onConfirm(action)} htmlType={"button"}>{busy && busy !== 'load' ? '处理中…' : buttonLabel}</Button></footer>}>
    <div className="modal-body cluster-rollback-preview">
      {busy === 'load' ? <LoadingBlock label="正在核对 Run、构建记录和安装基线…"/> : error && !lifecycle ? <><ErrorBlock message={error} onRetry={onRetry}/><StatusExplanationPanel explanation={explanation} title="环境生命周期操作被阻断"/></> : lifecycle ? <>
        ...
        {!blocked && <Field label={"输入环境名称以确认"} extra={<><small>必须完整输入：{environment.name}</small></>}><Input aria-label="确认环境名称" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={environment.name} autoComplete="off"/></Field>}
      </> : null}
    </div>

  </Modal>;
}
function ClusterRollbackModal({ environment, plan, error, explanation, busy, onRetry, onClose, onConfirm }: {
    environment: Environment;
    plan?: EnvironmentRollbackPlan;
    error?: string;
    explanation?: WorkExplanation;
    busy?: 'preview' | 'submit';
    onRetry: () => void;
    ...
    return <Modal size="wide" title="重置环境" description={`目标环境：${environment.name}。恢复全部集群节点，保留 File Station 和镜像仓库。`} onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={busy !== undefined} onClick={onClose} htmlType={"button"} type="default">{empty ? '关闭' : '取消'}</Button>{!empty && <Button className="button button--danger" disabled={!plan || !plan.planDigest || !confirmed || busy !== undefined} onClick={() => onConfirm(confirmation)} htmlType={"button"} type="default" danger>{busy === 'submit' ? '正在锁定计划…' : '创建重置 Run（待审批）'}</Button>}</footer>}>
    <div className="modal-body cluster-rollback-preview">
      ...
      {busy === 'preview' ? <LoadingBlock label="正在校验安装来源、备份基线和 Playbook 指纹…"/> : error ? <><ErrorBlock message={error} onRetry={onRetry}/><StatusExplanationPanel explanation={explanation} title="重置操作被阻断"/></> : empty ? <EmptyState title="暂无需要重置的集群组件" description="平台没有记录需要恢复的集群安装基线或未完成操作，无需创建重置 Run。"/> : plan ? <>
        ...
        <Field label={"输入环境名称以确认"} extra={<><small>必须完整输入：{environment.name}</small></>}><Input aria-label="确认重置环境名称" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={environment.name} autoComplete="off"/></Field>
      </> : null}
    </div>

  </Modal>;
}
function ChangeReasonModal({ title, description, warning, diffLines, busy, onClose, onConfirm }: {
    title: string;
    description: string;
    warning?: string;
    diffLines: string[];
    ...
    return <Modal title={title} description={description} onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--primary" disabled={busy || !reason.trim()} onClick={() => onConfirm(reason.trim())} htmlType={"button"} type="primary">{busy ? '处理中…' : '确认创建版本'}</Button></footer>}><div className="modal-body revision-preview"><div className="revision-diff"><GitCompare size={18}/><div><strong>变更预览</strong>{diffLines.map((line) => <p key={line}>{line}</p>)}</div></div>{warning && <div className="inline-warning"><AlertTriangle size={17}/><span>{warning}</span></div>}<Field label={"变更原因"}><Input.TextArea rows={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="例如：调整 node-6 地址并补充 FILE_STATION" autoFocus/></Field></div></Modal>;
}
...
function EnvironmentImportModal({ environments, selected, onClose, onDone }: {
    ...
    async function preview() {
        ...
        try {
            ...
            if (parsed.formatVersion !== 'clusterforge-environment/v1')
                throw new Error('仅支持 clusterforge-environment/v1 文件。');
            const result = await api.previewEnvironmentImport(request(parsed));
            setDocument(parsed);
            setPlan(result);
        }
        catch (reasonValue) {
            notify('error', '环境导入预检失败', reasonValue instanceof SyntaxError ? '导入内容不是有效 JSON。' : displayError(reasonValue));
        }
        finally {
            setBusy(undefined);
        }
    ...
    async function submit() {
        ...
        try {
            ...
            notify('success', targetKind === 'new' ? '已从文件创建环境' : `已创建环境版本 r${plan.nextRevision}`, '运行、健康记录、安装证据和审计历史没有被导入。');
            onDone(environment);
        }
        catch (reasonValue) {
            setPlan(undefined);
            notify('error', '环境导入失败', displayError(reasonValue));
        }
        finally {
            setBusy(undefined);
        }
    ...
    return <Modal busy={Boolean(busy)} size="wide" title="导入环境版本" description="导入只复用环境配置；不会复制 Run、健康检查、安装记录、备份或审计历史。" onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={Boolean(busy)} onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--quiet" disabled={Boolean(busy) || !text.trim() || !reason.trim() || (targetKind === 'new' ? !name.trim() : !targetEnvironmentId)} onClick={() => void preview()} htmlType={"button"} type="default">{busy === 'preview' ? '预检中…' : '预览差异'}</Button><Button className="button button--primary" disabled={Boolean(busy) || !plan} onClick={() => void submit()} htmlType={"button"} type="primary">{busy === 'submit' ? '导入中…' : '确认创建版本'}</Button></footer>}>
    <div className="modal-body">
      <Field label="选择环境文件"><FilePicker accept="application/json,.json" onChange={file => {
        void file.text().then(value => { setText(value); setPlan(undefined); setDocument(undefined); setConfirmReferences(false); });
      }}/></Field>
      <Field label="环境导出 JSON"><Input.TextArea className="code-editor" rows={12} value={text} onChange={event => { setText(event.target.value); setPlan(undefined); setDocument(undefined); setConfirmReferences(false); }} placeholder="粘贴 clusterforge-environment/v1 文件；缺少的 CredentialRef reference 可直接在此映射或删除。"/></Field>
      <Field label={"导入目标"}><Select value={targetKind} onChange={(selectedValue) => { setTargetKind(selectedValue as 'new' | 'existing'); setPlan(undefined); }} popupMatchSelectWidth={true}><Select.Option value="new">创建新环境及 r1</Select.Option><Select.Option value="existing">既有环境创建新版本</Select.Option></Select></Field>
      {targetKind === 'new' ? <div className="form-grid"><Field label={"新环境名称"}><Input value={name} onChange={(event) => { setName(event.target.value); setPlan(undefined); }}/></Field><Field label={"说明"}><Input value={description} onChange={(event) => { setDescription(event.target.value); setPlan(undefined); }}/></Field></div> : <Field label={"目标环境"}><Select value={targetEnvironmentId} onChange={(selectedValue) => { setTargetEnvironmentId(selectedValue); setPlan(undefined); }} popupMatchSelectWidth={true}><Select.Option value="">请选择</Select.Option>{environments.map((environment) => <Select.Option key={environment.id} value={environment.id}>{environment.name} · r{environment.currentRevision?.revision ?? 1}</Select.Option>)}</Select></Field>}
      <Field label={"变更原因"}><Input.TextArea rows={2} value={reason} onChange={(event) => { setReason(event.target.value); setPlan(undefined); }} placeholder="说明本次导入用途"/></Field>
      {containsReferences ? <Checkbox className="checkbox-field" checked={confirmReferences} onChange={(event) => { setConfirmReferences(event.target.checked); setPlan(undefined); }}><span>我确认复用文件中的 CredentialRef 引用；文件不包含 Secret 实际值</span></Checkbox> : null}
...
function CreateEnvironmentModal({ onClose, onDone }: {
    onClose: () => void;
    onDone: () => void;
}) {
    ...
    async function submit(values: Record<string, string>) {
        ...
        try {
            ...
            notify('success', '环境已创建', '请继续配置 Inventory 和 CredentialRef。');
            onDone();
        }
        catch (reason) {
            notify('error', '创建环境失败', displayError(reason));
        }
        finally {
            setBusy(false);
        }
    }
    return <Modal busy={Boolean(busy)} title="新建共享环境" description="环境标签来自平台统一目录，每个分类选择一个实际值。" onClose={onClose} formProps={{ onFinish: (values) => submit(values), layout: "vertical", preserve: false }} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--primary" disabled={busy || factDimensions.some((dimension) => platformOptionCategories.find((category) => category.id === dimension.id)?.environmentRequired && !facts[dimension.key])} htmlType={"submit"} type="primary">{busy ? '创建中…' : '创建环境'}</Button></footer>}><div className="form-grid"><Field label={"环境名称"} name={"name"} required rules={[{ required: true, message: "请填写此项" }]}><Input required placeholder="集群测试环境"/></Field><Field className="span-2" label={"说明"} name={"description"}><Input.TextArea rows={3}/></Field></div><EnvironmentFactFields categories={platformOptionCategories} facts={facts} editable onChange={setFacts}/></Modal>;
}
