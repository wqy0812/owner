...
export function EnvironmentsPage() {
  ...
  async function restore(reason: string) {
    ...
    try {
      ...
      signalRefresh(['environments', 'workbench']);
    } catch (errorReason) { notify('error', '版本恢复失败', displayError(errorReason)); } finally { setBusy(false); }
  }
  ...
  return <div className="page">
    <PageHeader eyebrow="Execution environments" title="环境管理" description="分别查看调度占用与真实连通性；配置变更以可追溯版本保存。" actions={user.role === 'environment_owner' ? <><button className="button button--quiet" onClick={() => setImportOpen(true)}><Upload size={16} /> 导入环境</button><button className="button button--primary" onClick={() => setCreateOpen(true)}><Plus size={16} /> 新建环境</button></> : undefined} />
    ...
    {loading && !environments ? <LoadingBlock label="正在读取共享环境…" /> : error && !environments ? <ErrorBlock message={error} onRetry={() => void reload()} /> : <div className="catalog-layout">
      ...
      {selected ? <section className="detail-stack">
        ...
        <article className="panel environment-editor">
          ...
          {editable && <footer className="editor-footer"><span className={dirty ? 'editor-dirty' : ''}><LockKeyhole size={14} /> {dirty ? '当前页有未保存更改' : '当前页与已保存版本一致'}</span><div>{dirty && <button className="button button--quiet" onClick={discardCurrent}>放弃本页更改</button>}<button className="button button--primary" disabled={busy || !dirty || (tab === 'parameters' && (parameterFields === undefined || staleParameterKeys.length > 0))} onClick={() => setSaveOpen(true)}><Save size={16} /> 保存新版本</button></div></footer>}
        </article>

        <article className="panel revision-history">
...
function EnvironmentImportModal({ environments, selected, onClose, onDone }: { environments: Environment[]; selected?: Environment; onClose: () => void; onDone: (environment: Environment) => void }) {
  ...
  return <Modal size="wide" title="导入环境版本" description="导入只复用环境配置；不会复制 Run、健康检查、安装记录、备份或审计历史。" onClose={onClose}>
    <div className="modal-body">
      ...
      <label><span>导入目标</span><select value={targetKind} onChange={(event) => { setTargetKind(event.target.value as 'new' | 'existing'); setPlan(undefined); }}><option value="new">创建新环境及 r1</option><option value="existing">既有环境创建新版本</option></select></label>
