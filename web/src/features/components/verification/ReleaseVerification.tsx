...
export function EvidenceLink({ run }: {
    run?: EvidenceSummary;
}) {
    if (!run)
        return <small className="verification-evidence verification-evidence--missing">暂无 Run 证据</small>;
...
export function ReleaseRunEvidenceModal({ release, onClose }: {
    ...
    return <Modal size="wide" title={`Run 证据 ${release.version}`} description="组件验证与场景执行均绑定到该不可变 Release；点击记录可进入运行中心查看完整步骤和日志。" onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={onClose} htmlType={"button"} type="default">关闭</Button></footer>}>
...
export function TestReleaseModal({ release, onClose, onDone }: {
    release: ComponentRelease;
    onClose: () => void;
    onDone: () => void;
}) {
    ...
    async function run() { if (!plan)
        return; setBusy('submit'); try {
        await api.testRelease(release.id, input(plan.planDigest));
        notify('success', '验证已提交', '可在运行中心查看各阶段结果。');
        onDone();
    }
    catch (reason) {
        invalidate();
    ...
    return <Modal busy={Boolean(busy)} title={`环境验证 ${release.version}`} description="先预览锁定计划，确认提交后创建单个 Ansible 作业。" onClose={onClose} footer={<footer className="modal-actions"><Button className="button button--quiet" disabled={!!busy} onClick={onClose} htmlType={"button"} type="default">取消</Button><Button className="button button--primary" disabled={!!busy || !plan} onClick={() => void run()} htmlType={"button"} type="primary">确认提交验证</Button></footer>}><div className="modal-body">
    <Field label={"验证动作"}><Select aria-label="验证动作" value={selection} onChange={(selectedValue) => { setSelection(selectedValue); invalidate(); }} popupMatchSelectWidth={true}><Select.Option value="">请选择动作</Select.Option>{release.parentReleaseId && <Select.Option value="evolution">升级闭环：父版本安装 → 升级 → 回滚</Select.Option>}<Select.OptGroup label="执行动作">{release.actions?.filter(action => action.type !== 'check').map(action => <Select.Option key={action.id} value={action.id}>{action.name || action.type}（含已安排的检查）</Select.Option>)}</Select.OptGroup><Select.OptGroup label="独立检查">{release.actions?.filter(action => action.type === 'check').map(action => <Select.Option key={action.id} value={action.id}>{action.name}</Select.Option>)}</Select.OptGroup></Select></Field>
    <Field label={"目标环境"}><Select aria-label="目标环境" value={environmentId} onChange={(selectedValue) => { setEnvironmentId(selectedValue); invalidate(); }} popupMatchSelectWidth={true}><Select.Option value="">请选择环境</Select.Option>{environments?.map(env => <Select.Option key={env.id} value={env.id}>{env.name}</Select.Option>)}</Select></Field>
