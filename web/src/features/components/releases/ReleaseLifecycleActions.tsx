...
export function useReleaseLifecycleActions({ scopeKey, onRestored, onDeleted }: {
    ...
    async function restoreRelease(release: ComponentRelease) {
        const generation = scopeGeneration.current;
        if (busy) return;
        if (!await confirm(`确认恢复 ${release.version}？\n该版本将重新成为可编辑 Draft；若发布线已有新的 Draft 或后继版本，后端会拒绝恢复。`))
            return;
    ...
    async function deleteRelease(release: ComponentRelease) {
        const generation = scopeGeneration.current;
        if (busy) return;
        if (!await confirm(`确认永久删除 ${release.version}？\n将删除该未发布 Release 的合同、Action、托管 Playbook、介质和镜像登记。\n\n只有已废弃、从未发布且没有 Run、构建或任何引用的 Release 可以删除；此操作不可恢复。`))
            return;
        if (generation !== scopeGeneration.current) return;
        setBusy(true);
        try {
    ...
    return {
        ...
        dialogs: <>
      {publishRelease && <Modal busy={Boolean(busy)} title={`发布 ${publishRelease.version}`} description={publishRelease.parentReleaseId ? '发布后版本不可修改；只通知精确锁定父 Release 的下游 Owner。' : '这是全新发布线，不替换现有锁定 Release，也不发送影响通知。'} onClose={closeDialogs} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={closeDialogs} htmlType={"button"} type="default">取消</Button><Button disabled={busy || !impact} className="button button--primary" onClick={() => void confirmPublish()} htmlType={"button"} type="primary">{busy ? '发布中…' : impact?.changeKind === 'new_line' ? '确认发布新线' : '确认发布并通知'}</Button></footer>}>
      ...
      {deprecateRelease && <Modal busy={Boolean(busy)} title={`${deprecateRelease.state === 'draft' ? '废弃草稿' : '废弃'} ${deprecateRelease.version}`} description={deprecateRelease.state === 'draft' ? '未发布草稿可以随时废弃；废弃后仍可恢复为 Draft，或在没有 Run、构建和引用时永久删除。' : '没有场景 Run 锁定时，该版本可停止作为推荐版本。'} onClose={closeDialogs} footer={<footer className="modal-actions"><Button className="button button--quiet" onClick={closeDialogs} htmlType={"button"} type="default">取消</Button><Button disabled={busy || !impact || deprecationBlocked} className="button button--danger" onClick={() => void confirmDeprecate()} htmlType={"button"} type="default" danger>{busy ? '废弃中…' : deprecateRelease.state === 'draft' ? '确认废弃草稿' : '确认废弃版本'}</Button></footer>}>
        <div className="modal-body">
