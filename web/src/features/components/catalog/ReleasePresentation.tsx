...
export function CompatibilityBadge({ release }: {
...
export function ReleaseReviewDetails({ release }: {
    release: ComponentRelease;
}) {
    const { users } = useApp();
...
export function DraftReadiness({ release, evidence, onContract, onReview, onLifecycle, onImage, onArtifact, onValidate, onPublish }: {
    ...
    {current && <section id={detailId} className="readiness-detail" aria-label={`${current.name}详情`}>
      <header><h3>{current.name}</h3><Button className="icon-text" onClick={() => setExpanded(undefined)} htmlType={"button"} type="text">收起详情</Button></header>
      {current.detail}
      ...
      {current.action && <Button className="button button--quiet" onClick={current.onAction} htmlType={"button"} type="default">{current.action}</Button>}
    </section>}
    <footer className="readiness-actions">
      <div><span>可选交付物：</span><Button className="icon-text" onClick={onImage} htmlType={"button"} type="text"><Container size={14}/> 镜像构建</Button><Button className="icon-text" onClick={onArtifact} htmlType={"button"} type="text"><Archive size={14}/> 管理介质</Button></div>
      <div><span>{ready ? '发布前将展示完整下游影响。' : '完成所有阻断项后才能发布。'}</span><Button className="button button--primary" disabled={!ready} title={ready ? undefined : '请先完成生命周期、安装验证和回退验证'} onClick={onPublish} htmlType={"button"} type="primary"><Rocket size={15}/> 预览影响并发布</Button></div>
    </footer>
  </article>;
}
export function EnvironmentConstraints({ constraints }: {
    ...
    if (!groups.length)
        return <div className="env-constraints env-constraints--empty">不限制适配范围</div>;
    return <div className="env-constraints">{groups.map((group) => (<span className="env-constraint" key={group.key}>
      <em>{group.label}</em>
