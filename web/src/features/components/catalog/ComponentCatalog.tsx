...
export function ComponentCatalog({ components, selected, detailId, editRelease, onSelect, children }: {
    ...
    const filteredComponents = useMemo(() => {
        ...
        return (components ?? []).filter((component) => {
            ...
            if (!matchesQuery)
                return false;
            if (catalogFilter === 'mine')
                return component.ownerId === user.id;
            if (catalogFilter === 'draft')
                return component.ownerId === user.id && component.hasDraft;
            if (catalogFilter === 'attention')
                return component.ownerId === user.id && componentNeedsAttention(component);
            return true;
        });
    ...
    function isLayerOpen(layer: ComponentLayer) {
        if (layerOpen[layer] !== undefined)
            return layerOpen[layer];
    ...
    async function selectComponent(id: string) {
        if (!await onSelect(id))
            return;
        const component = components?.find(item => item.id === id);
        if (component)
            setLayerOpen(current => ({ ...current, [component.layer]: true }));
    }
    ...
    <aside className="catalog-list panel">
      <div className="catalog-list__header">
        ...
        <div className="catalog-list__tools">
          ...
          <Button className="icon-text" onClick={() => setAllLayers(true)} aria-label="全部展开" title="全部展开" size="small" htmlType={"button"} type="text"><ChevronDown size={16}/></Button>
          <Button className="icon-text" onClick={() => setAllLayers(false)} aria-label="全部折叠" title="全部折叠" size="small" htmlType={"button"} type="text"><ChevronRight size={16}/></Button>
        </div>
      </div>
      ...
      <div className="catalog-filters" aria-label="组件目录筛选">
        ...
        {([
            ...
            CatalogFilter,
            string
        ]>).map(([value, label]) => <button key={value} type="button" className={catalogFilter === value ? 'active' : ''} aria-pressed={catalogFilter === value} title={value === 'attention' ? '我负责的组件中，尚无版本或有 Draft 的组件' : undefined} onClick={() => setCatalogFilter(value)}>{label}</button>)}
      </div>
      ...
      {components?.length && filteredComponents.length ? layeredComponents.map(({ layer, components: layerComponents }) => {
            if ((catalogSearch || catalogFilter !== 'all') && !layerComponents.length)
                return null;
          ...
          {open ? <div id={`catalog-layer-${layer.value}`}>{layerComponents.length ? layerComponents.map((component) => (<button key={component.id} type="button" disabled={Boolean(editRelease) && selected?.id !== component.id} className={`catalog-item${detailId === component.id ? ' active' : ''}`} onClick={() => selectComponent(component.id)}>
            ...
            </button>)) : <div className="catalog-layer__empty">本层暂无组件</div>}</div> : null}
        </section>;
