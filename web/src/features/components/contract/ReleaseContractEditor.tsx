...
export function ReleaseContractEditor({ release: initialRelease, components, focusSection = 'dependencies', onCancel, onSaved, onDirtyChange }: {
    release: ComponentRelease;
    components: Component[];
    focusSection?: ContractSection;
    onCancel: () => void;
    onSaved: () => void;
    ...
    async function cancel() {
        if (!dirty || await confirm('当前区域有未保存修改，确认放弃？'))
            onCancel();
    }
    function addReference() {
        const selected = sourceOptions.find(item => JSON.stringify([item.dependency.releaseId, item.parameter.name]) === source);
        if (!selected || !targetName.trim() || !targetDescription.trim())
            return;
        if (parameters.some(parameter => parameter.name === targetName.trim())) {
            notify('error', '参数名已存在');
            return;
        }
    ...
    async function save() {
        ...
        try {
            ...
            notify('success', focusSection === 'parameters' ? '参数合同已保存' : '直接依赖已保存');
            onSaved();
        }
        catch (reason) {
    ...
    <article className="panel" id="contract-dependencies"><header className="panel__header"><div><span className="panel__icon panel__icon--cyan"><GitBranch size={18}/></span><div><h2>直接依赖</h2><p>{release.version} · {focusSection === 'dependencies' ? '编辑上游版本与公开参数映射' : '当前依赖，仅供参考'}</p></div></div></header>
      {focusSection === 'dependencies' ? <><div className="contract-editor"><DependencyEditor dependencies={dependencies} components={components} currentParameters={parameters} currentComponentId={release.componentId} onChange={setDependencies}/></div>
        <details className="contract-reference-create"><summary>新增引用参数</summary><p>将新的本组件参数与上游公开参数一起保存。</p><div className="form-grid"><Field label={"上游公开参数"}><Select value={source} onChange={(selectedValue) => setSource(selectedValue)} popupMatchSelectWidth={true}><Select.Option value="">请先添加依赖并选择公开参数</Select.Option>{sourceOptions.map(item => <Select.Option key={JSON.stringify([item.dependency.releaseId, item.parameter.name])} value={JSON.stringify([item.dependency.releaseId, item.parameter.name])}>{item.label}</Select.Option>)}</Select></Field><Field label={"本组件参数名"}><Input value={targetName} onChange={event => setTargetName(event.target.value)}/></Field><Field label={"参数说明"}><Input value={targetDescription} onChange={event => setTargetDescription(event.target.value)}/></Field><Field label={"可见性"}><Select value={targetVisibility} onChange={(selectedValue) => setTargetVisibility(selectedValue as 'internal' | 'public')} popupMatchSelectWidth={true}><Select.Option value="internal">内部</Select.Option><Select.Option value="public">公开</Select.Option></Select></Field></div><Button className="button button--secondary" disabled={!source || !targetName.trim() || !targetDescription.trim()} onClick={addReference} htmlType={"button"} type="default">添加引用参数</Button></details>
        {orphaned.length > 0 && <div className="inline-warning"><div><strong>以下引用参数已失去映射来源</strong>{orphaned.map(parameter => <p key={parameter.name}>{parameter.name} <Button className="icon-text" onClick={async () => {
                        if (await confirm(`确认一并移除引用参数 ${parameter.name}？保存时仍会校验其他引用。`))
