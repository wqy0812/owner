  ...
  return <section className="playbook-editor">
    ...
    {action ? <>
      ...
      <div className="form-grid playbook-action-fields">
        ...
        {action.type !== 'check' && <>
          {action.type !== 'rollback' && <label><span>前置检查 *</span><select aria-label="前置检查" value={action.preCheckActionId ?? ''} onChange={event => updateAction({ preCheckActionId: event.target.value })}><option value="">请选择本版本的检查动作</option>{actions.filter(item => item.type === 'check' && item.id).map(item => <option key={item.id} value={item.id}>{item.name || item.id}</option>)}</select></label>}
          ...
          <label><span>可安全重试</span><select aria-label="可安全重试" value={String(action.idempotent ?? false)} onChange={(event) => updateAction({ idempotent: event.target.value === 'true' })}><option value="false">否 · 未声明可安全重试</option><option value="true">是 · 已验证可安全重试</option></select><small>{action.type === 'rollback' ? '仅在已验证部分执行后可安全重跑时声明；回滚主体成功后只重试未通过的后检查。' : '仅在已验证部分执行后可安全重跑时声明；已绑定的前置检查会随动作重试。'}</small></label>
