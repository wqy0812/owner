...
export function useComponentDeletion({ scopeKey, onDeleted, onRejected }: {
  ...
  async function deleteComponent(component: Component) {
    if (!component.canDelete || component.ownerId !== user.id || user.role !== 'component_owner' || pending.current.has(scopeKey)) return;
    if (!window.confirm(`确认永久删除空组件“${component.name}”？\n仅没有任何版本且没有引用的组件可以删除；历史审计记录将保留。\n\n此操作不可恢复。`)) return;
