...
> 版本与环境：本文保留 2026-08-28 双版本组件录入时的 V1 设计方案。它不构成安装、测试、候选、发布或环境变更证据。

> 当前使用边界：下文数量、空参数/空动作和 Docker 约束是当时录入范围，不是当前发布门禁。现有代码统一使用组件/场景适配范围与环境实际值，运行时由精确组件依赖表达；开展新工作先读[平台说明书](platform-manual.md)及[开发指南](development-guide.md)。
