...
> 文档基线：2026-09-05 当前工作区代码；带日期的环境示例仍为历史快照
...
- 单击“保存 Playbook”会把完整 Action 配置、固定入口文件、工作区清单和合同失效作为一个操作立即保存；删除已保存 Action 同样立即原子删除。文件替换与 SQLite 提交之间由持久恢复标记保护，进程中断后会在服务开放前恢复旧入口。关闭 Draft 编辑器不会撤销这两类已完成操作，其余版本、参数和依赖修改仍需“保存 Draft”。
...
安装验证的主动作按 Upgrade → Install → Configure → Preflight → Inspect 的顺序选择第一个已定义动作；如果定义了 Verify，会自动追加 Verify。环境实际适配标签必须匹配 Release 支持范围。平台依据当前 Release 内容身份、Playbook 工作区摘要和成功 Run 实时计算 Readiness，不再按运行时版本维护专用矩阵。测试后修改 Draft 会让旧证据和平台 Owner 审核失效，并撤回候选共享。`testValue` 只证明组件能够独立消费映射参数，实际组件间传递必须通过场景完整测试。
...
场景 Revision 还要声明独立的多选适配范围，并被每个组件支持范围覆盖。标签与 DAG 原子保存：缺项可暂存，明确冲突不能保存；完整测试、发布和运行要求匹配且齐全，标签改变后测试证据失效。
