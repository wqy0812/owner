...
>
> 代码基线：2026-08-30 当前工作区。本文记录该日期的测试平台业务数据，不等于 `NEWPLATFORM_SEED_PROFILE=demo` 内置 Seed。
>
> 这是可复现快照，不是生产兼容矩阵。版本和环境规则见[首版与环境策略](version-policy.md)。
>
> 连通性证据边界：09:06/09:07 的 SSH 结果来自当时已部署的 Ansible Ping 检查器。该快照记录时的 Go SSH 检查器尚未在真实环境重新执行，部署后必须补充显式 SSH CredentialRef 并重新检查，不能沿用下文旧结果。

> 使用前先重新核对当前部署和结构合同。旧快照不保证能导入当前 V1，也不能代表现在的资产数量、凭据或连通性。当前规则见[平台说明书](platform-manual.md)。
