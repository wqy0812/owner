...
> 版本与环境：本文属于项目首个版本（V1）；所有操作目标均为测试环境，不是生产环境。V1 只接受当前精确合同，不提供历史迁移或双合同兼容；其他合同失败关闭。统一规则见 [首版与环境策略](version-policy.md)。

> 文档基线：2026-08-30 当前工作区代码与测试环境只读快照
> 适用对象：组件 Owner、集群 Owner、环境 Owner 及演示平台 Owner
...
| 显示身份 | 角色 | 主要职责 |
| --- | --- | --- |
| 林晓 | 组件 Owner | 当前 15 个组件及其 Release |
| 陈晨 | 集群 Owner | 场景 DAG、完整测试和发布 |
| 王维 | 环境 Owner | 环境、Inventory、凭据引用和危险作业审批 |
...
| 集群 Owner | 自己的当前可变 Revision、图校验和完整测试状态、发布入口、按场景聚合的未读上游影响 |
...
- 组件参数所有权合同和环境约束。
- 场景节点 `parameterValues`、环境参数和上游映射。
- 环境 Facts 和 Variables。

密码、token、私钥、加密密钥等必须通过环境的“凭据引用”配置。不要把实际 secret 粘贴到参数字段、组件发布说明或环境变量中。
...
- 刷新后重新确认 Revision、节点锁定 Release、连线、只读主机组和 parameterValues，再执行校验或测试。
...
- `NEWPLATFORM_SEED_PROFILE=identities` 写入身份，并只在新库创建一次平台治理目录；不写入组件、场景、环境或场景模板。后续启动保留平台管理员的删除、重命名和停用结果。
...
1. 选择自己拥有的组件。
2. 单击“创建 Draft”，选择“全新发布线”或“基于现有发布线演进”。
3. 全新发布线填写线名，可空白创建或复制同组件 Released/Deprecated 版本作为模板；空白创建从空环境约束开始，选择模板后复制模板约束。演进只能选择该线最后一个曾发布且当前仍为 Released、没有保留后继的父版本，并继承其环境约束，再声明“兼容升级”或“破坏性升级”。若最后一个曾发布版本已废弃，页面显示阻断原因并要求创建新发布线。
4. 预览计划并提交 `expectedPlanDigest` 后创建 Draft。

全新发布线不与任何现有版本建立升级关系。复制模板时保留依赖、参数、非 Upgrade Action、Playbook、介质和镜像内容身份，但会清除 Upgrade 和 Rollback 的旧版本绑定。发布线保持线性，每条线最多一个有效 Draft；不同发布线可分别存在 Draft。

同一组件需要投放到不同主机组时，由组件 Owner 分别创建全新发布线，并在各发布线的 Action 中固定主机组。可以复制已有 Release 以复用 Playbook、参数和制品合同；场景 Owner 只能选择这些精确 Release，不能把某条发布线的主机组改成另一个目标。
...
- 版本、发布说明、Release 风险级别和兼容性。根版本固定显示“全新基线”；演进版本显示“兼容升级”或“破坏性升级”。
- 结构化环境约束：架构、操作系统、操作系统版本、网络栈，以及容器运行时/运行时版本。Release 可声明多组明确组合，必须先选运行时，再按运行时分组选择版本。
- 结构化参数表：名称、说明、类型、必填、可见性、是否可修改、值提供方、固定值、建议值、测试值、环境绑定、枚举和最小长度。可见性必须显式选择 `internal` 或 `public`；值提供方必须选择组件、场景、环境或上游映射。场景或环境 Owner 的结构化字段必须有受控枚举，不能让外部 Owner 编辑原始 JSON。
...
- 结构化生命周期动作及在线 Playbook 编辑器：动作类型、路径、tags、主机组、凭据引用、超时、风险、幂等及版本转换端点分别填写。Action 不再声明自由运行参数。
...
示例公开参数：`kubeInstallRoot`，类型 string，可见性 public，由组件提供且 `fixedValue=/approot1/paas/kube`。下游 kube-proxy 把它映射到 `valueProvider=upstream_mapping` 的 `kubeRoot`。首版不支持 Playbook 执行后动态产生的参数输出。
...
- 保存参数、Action、环境绑定或 Playbook 会使之前的平台 Owner 审核和组件测试证据失效，需要重新提交审核和测试。

### 3.5 构建并发布镜像

1. 确认目标环境的环境 Owner 已在“环境变量”中配置 `IMAGE_REGISTRY`。
...
3. 选择共享环境。组件固定参数直接使用 `fixedValue`；映射参数使用 Release 中的 `testValue`；环境参数从所选 Environment Revision 读取。
4. 单击“预览执行计划”，确认每一步所属版本、Playbook、目标主机组、参数来源和审批要求。目标环境已有相同内容时直接复用；目标缺失但来源可读时会列出逐项交付要求。
5. 只有预览成功后才能确认提交；任何环境 Revision、交付目标、目标版本或合同变化都会使旧计划失效，需要重新预览。
6. 需要审批时，环境 Owner 对每项内容选择“直接使用来源”或“平移到目标”。介质由目标 FSS 主动拉取并校验 SHA-256，镜像按 digest 平移；审批期间目标内容出现时执行器直接复用。
7. 进入“运行”查看状态、步骤、锁定的交付选择、实际交付结果、解析参数来源和日志。

安装验证的主动作按 Upgrade → Install → Configure → Preflight → Inspect 的顺序选择第一个已定义动作；如果定义了 Verify，会自动追加 Verify。环境先单选容器运行时，再单选该运行时的版本；切换运行时会清空旧版本。Release 如声明多个运行时/版本组合，每个组合都必须分别完成安装/Verify 和 rollback/Verify，缺少任一矩阵行都会阻止 Candidate 与发布。平台依据当前 Release 内容身份、Playbook 摘要、环境运行时快照和成功 Run 实时计算 Readiness；测试后再修改 Draft 会让旧证据和平台 Owner 审核立即失效，并撤回候选共享。`testValue` 只证明组件能够独立消费映射参数，实际组件间传递必须通过场景完整测试。
...
组件 Owner 可在任意自有 Release 的发布行单击“Run 证据”。弹窗分别列出该 Release 的全部组件验证历史，以及所有在不可变执行快照中锁定该 Release 的场景测试和正式运行。场景记录按稳定 Scenario 聚合，跨 Revision 仍归入同一场景；展开后可按时间查看成功、失败、取消、审批中或运行中的每次 Run，并进入运行中心查看完整步骤和日志。此入口只读取历史证据，不改变 Readiness 或 Release 状态。

### 3.8 发布组件版本

1. 找到 Draft，确认参数和 Action 合同已经保存，单击“提交平台 Owner 审核”。无参数 Release 也必须审核。
2. 平台 Owner 在“我的工作”打开该条待办的“预览并审核”，核对完整合同、全部托管 Playbook 正文及实际 SHA-256 后批准或驳回；批准备注可选，驳回原因必填。预览失败或摘要漂移时不能提交决定，必须重新预览。平台管理只负责平台目录与全局字段维护，不再提供合同审核入口。
3. 审核通过后查看影响预览：下游组件 Owner、集群 Owner、受影响场景和依赖路径。
4. 检查兼容性、生命周期完整性，以及新基线证据或演进闭环证据。
5. 单击“确认发布”。全新基线不发送影响通知；演进版本只通知精确锁定父 Release 的下游。
...
- 环境约束是否与支持范围一致。
- 必填参数是否可解析。
- Playbook、tags、Action 主机组和 timeout 是否正确。
- 危险动作是否显式标注。
- Upgrade/Rollback 起止版本是否正确。
- 发布说明是否足够让下游判断影响。

前台和 `POST /component-releases/{id}/publish` 使用同一门禁：新基线必须定义 install、verify 和无版本绑定的清理型 rollback，并具备相应成功证据；演进版本必须具备显式 Upgrade 或幂等 Install、精确绑定回父版本的 Rollback，并在一次锁定计划中完成“父版本安装与验证 → 目标升级与验证 → 回退 → 父版本验证”。规格摘要过期等同缺少证据。直接发布还要求所有上游已经 Released。
...
需要与一组相互依赖的 Draft 一起交付时，不要逐个直接发布：每个组件 Owner 先取得平台 Owner 审核批准，并在 Readiness 满足后单击“加入候选集”。该显式交接会让集群 Owner 看见 Draft 及统一阻断原因；场景经完整测试后使用“预览候选集并发布”，把 Scenario Revision 与全部候选 Release 原子发布。组件 Owner 可使用“撤回候选”。后续参数、Action、环境绑定或 Playbook 修改会立即清空审核和候选状态，必须重新审核并完成证据。

### 3.9 废弃、恢复或永久删除组件版本

未发布 Draft 可随时单击“废弃草稿”。废弃不会删除 Release、Playbook、介质登记或 Run 证据，而是把草稿转为只读的 Deprecated 状态并退出候选共享。

对从未发布的 Deprecated Release，组件 Owner 有两个后续选择：

1. 单击“恢复 Draft”，确认后继续编辑；若同一发布线已有新的 Draft，或同一父版本已有新的有效后继，恢复会被后端拒绝。
2. 单击“永久删除”，再次确认不可恢复操作。只有不存在任何 Run、镜像构建、组件/场景/版本合同引用和环境安装记录时才会删除；系统同时清理 Release 自有合同与托管 Playbook，并保留删除审计。外部文件站介质和 Registry 镜像本体不会被删除，只移除本 Release 的登记。

已发布 Release 只能单击“废弃”。

已发布 Release 的废弃不会删除历史。仅被场景引用但从未运行的版本仍可在影响确认后废弃；一旦场景测试或正式 Run 的不可变执行快照锁定了该 Release，平台将拒绝废弃，以保留运行记录和交付依据。已发布版本无论是否废弃都不能恢复为 Draft 或永久删除。

### 3.10 处理组件影响通知

1. 进入“通知”。
2. 查看父版本、兼容升级或破坏性升级标记，以及精确 Release 影响路径。
...
如果场景是误建的，并且所有 Revision 都从未发布、也从未产生任何测试或正式 Run，集群 Owner 可以单击“删除场景”并确认。系统会永久删除该场景和未发布 Revision；只要已有发布或运行历史，后端就会拒绝删除，此时应使用“废弃”保留交付与审计依据。
...
3. 选择节点，在右侧配置：
   - 显示名称。
   - 生命周期动作。
   - 主机组（从所选 Release Action 继承，只读）。
   - 依赖参数来源：唯一上游自动绑定，多个可达节点时必须选择。
   - 仅属于集群 Owner 的结构化参数字段。
4. 也可以切换到“参数总览”，按组件 → 精确 Release → 节点集中填写；同一 Release 的多个节点分别保存值。
5. 单击“保存草稿”。DAG 与全部节点 `parameterValues` 在同一事务提交，任一字段失败都不会部分落库。
...
含义：仅 `modifiable=true` 且 `valueProvider=scenario_owner` 的 `cpu` 进入节点。建议值必须单击“采用建议值”或人工输入才算完成；组件固定值、环境字段和上游映射不会出现在填写区。
...
| 必填参数未绑定 | 场景字段在参数总览填写；环境字段由环境 Owner 填写；映射字段确认唯一可达来源 |
| 需要选择依赖来源 | 在重复上游节点中明确选择 sourceNodeId |
| 失效字段 | Release 合同已变化，确认清理旧字段并补齐新的必填字段 |

只有当前集群 Owner 可以校验自己的 Draft；后端会再次校验，不依赖画布前端判断。
...
2. 单击“环境测试”。
3. 选择共享环境。
4. 单击“开始完整测试”。运行接口只提交 Environment，不接受临时 Run 参数。
5. 在“运行”查看拓扑步骤、自动 Verify 和日志。
...
2. 单击“环境运行”。
3. 选择包含完整环境参数的目标 Environment Revision；不存在临时运行覆盖。
4. 提交后到“运行”跟踪。
...
1. 选择自己拥有的环境。
2. 打开“Inventory”页签。
3. 单击“添加节点”，在弹框中填写节点名称、地址、SSH 用户和端口，并勾选至少一个主机组。已有节点可通过行尾按钮编辑或移除。
4. 单击“主机组管理”，左侧选择主机组，右侧查看组内节点；在“添加节点”中搜索环境已有节点并逐台或批量加入。移出主机组只解除分组关系，节点仍保留在环境中。
5. 单击“应用更改”回到页面，再单击“保存新 Revision”，核对差异并填写变更原因后确认。取消弹框会放弃本次分组修改。

要求：

- 主机名和地址不能为空。
- 每台主机至少属于一个组。
- 节点名称不能重复，SSH 端口范围为 1–65535。
- 主机组名称由平台管理员维护；已退役主机组保留展示，不能新增成员。
...
1. 打开“环境事实”。
2. 按平台当前的环境维度类别逐项选择目录选项；容器运行时必须先选运行时，再从过滤后的版本中单选。已退役的历史事实只读保留，不能新增或改成其他退役值。
...
平台会用这些事实匹配组件环境约束。必填集合由当前平台目录决定；新增类别可以指定父类别与是否必填。父子选项必须成对有效，版本不会在多个运行时之间做笛卡尔积。平台 Owner 可退役/恢复类别和选项；退役引用保留在历史快照中，但新数据不能新增，既有对象修改时也只能原样保留。

### 5.4 配置组件环境参数与非敏感环境变量

组件环境参数：

1. 打开“组件环境参数”。页面只展示 Component Release 分配给环境 Owner 的字段，并列出引用它的组件和精确版本。
2. 全局字段会预填平台默认值，环境 Owner 可以修改，也可以单击“采用平台默认值”。平台未设置默认值的全局字段必须填写；`false` 和 `0` 都是有效值。组件私有字段的建议值仍需单击采用或人工填写。
3. 保存新 Revision 时检查全局字段是否完整，实际值（含采用的默认值）会保存到环境快照。环境 Owner 不能填写场景字段、组件固定字段、映射字段或未定义键。

平台 Owner 在“平台管理 → 全局环境参数字段”创建字段时，可以勾选“设置平台默认值”；已有字段通过“编辑默认值”设置、修改或取消默认值。默认值必须满足字段类型、枚举和最小长度，取消默认值后由环境 Owner 必填。修改默认值不会覆盖已有环境值、历史 Revision 或 Run。新建环境先记录可用默认值，缺少默认值的字段在“组件环境参数”中补齐；运行时不会临时读取平台默认值。

组件草稿删除参数后，环境页会列出仍然保存的失效值。环境 Owner 可逐项移除，检查差异后保存新 Revision；放弃本页更改可以撤销这些移除，历史 Revision 保持原值。

正式组件测试、场景测试或运行只校验其锁定 Release 实际需要的环境字段；缺少必填值时运行在排队前失败关闭。

非敏感环境变量：

1. 打开“环境变量”。
2. 单击“添加变量”，从平台 Owner 维护的字段目录选择变量名并填写字符串值，不能创建任意键。
3. 保存新 Revision，核对差异并填写变更原因后确认。
...
不要把密钥值写入 `.env.example`、Seed、环境 Parameters 或 Variables。
...
平台先按来源 Run 的完成时间倒序排列安装层，再读取每个来源的锁定节点计划，过滤安装类步骤并反转原执行顺序。这样后安装的细粒度场景会先恢复到更早场景的状态，随后更早场景再恢复到其安装前状态。控制面和工作节点复用同一组件时分别锁定组件 Owner 提供的主机组发布线；平台只在该组件最后一个节点回滚成功后删除安装记录和远端备份。
...
同一维护窗口有多条待审批 Run 时，可在运行中心右上角使用“批量审批”。弹窗会列出每条 Run 的环境和风险，必须填写审批理由；前端与 `POST /approvals/batch` 都会拒绝空白理由，后端保存裁剪后的统一理由并原子消费整批审批。任一记录过期或不属于当前环境 Owner 时整批失败，批准后仍按每个环境的 FIFO 队列串行执行。
...
归档环境只在环境 Owner 的环境管理列表中显示“已归档”，所有写操作关闭。单击“恢复环境”、再次完整输入环境名称后，环境重新参与新任务选择；恢复不会修改任何历史记录。

### 5.11 发布目录私有 Git 灾备

环境 Owner 在“灾备目录 → 发布目录灾备”中输入服务器本地路径，可以在平台允许根目录下创建权限为 `0700` 的私有 bare Git 仓库，也可以接入已经包含 `catalog` 分支的仓库。空目录首次恢复使用“从已有 Git 仓库恢复”向导，依次完成仓库验证、恢复点选择、预检和确认；只希望配置备份时可选择“仅接入，暂不恢复”。已有目录只能“接入已有备份仓库”，已经配置后使用“更换备份仓库”。发布后平台异步复制；页面列出 `backup/*` 恢复点，并显示发布代次、最近成功和最近错误。相同异常也会出现在环境 Owner 的“我的工作”。

页面同时展示完整备份策略：组件或场景发布、联合发布及已发布对象废弃后，30 秒内合并并异步备份；环境 Owner 可单击“立即备份”同步创建恢复点。立即备份期间不能切换仓库，只有 SQLite 完整性与外键、数据库/Catalog/Playbook SHA-256、`catalog` 分支 push 和不可变 `backup/*` 标签 push 全部成功后，页面才提示完成并刷新最近成功时间。人工备份不再通过 CLI 触发。
...
发布全新基线时，平台明确显示“不替换现有锁定版本”，不创建影响通知。发布线演进时，平台从父 Release ID 沿精确反向依赖链计算直接和传递影响，并通知：

- 受影响的下游组件 Owner。
- 精确锁定父 Release 或传递受影响 Release 的集群 Owner。

通知会显示父版本、目标版本、兼容性、影响路径和关联场景。可单条标记已读或“全部已读”；当前 Demo 不支持恢复未读。

## 8. 当前测试环境样例

2026-08-29 快照记录了 15 个已发布组件 Release、1 个已发布场景 Revision 和 2 个环境。该历史快照中的场景“Kubernetes 1.17.5 Ubuntu 六节点细粒度集群”为 r2，画布包含 21 个节点和 50 条依赖边。当前合同要求控制节点和工作节点使用组件 Owner 提供的不同 Release 发布线，场景 Owner 不再直接选择节点 `hostGroup`。
...
- 场景参数总览中的必填字段必须完整，所选 Environment Revision 也必须包含锁定 Release 所需的环境字段。

### 9.4 Run 一直等待审批

切换到目标环境的环境 Owner，在运行详情批准或拒绝。其他环境 Owner、组件 Owner 和集群 Owner 都不能审批。
...
环境不提供直接复制。环境 Owner 可导出指定 Revision：默认文件不含 CredentialRef 的 `reference`；显式敏感导出只包含引用字符串，仍不包含环境变量值或私钥内容。导入可以创建新环境 r1，或为既有环境创建新 Revision，不会带入 Run、健康检查、安装记录、备份和审计历史。平台按文件中实际存在的非空 `reference` 判断是否必须确认复用，不信任可编辑的摘要标志；预览规范化后的 Variables 与正式落库使用同一快照。
