...
3. 检查兼容性、生命周期完整性，以及新基线证据或演进闭环证据。
4. 单击“确认发布”。全新基线不发送影响通知；演进版本只通知精确锁定父 Release 的下游。
...
2. 单击“永久删除”，再次确认不可恢复操作。只有不存在任何 Run、镜像构建、组件/场景/版本合同引用和环境安装记录时才会删除；系统同时清理个人运行参数预设、Release 自有合同与托管 Playbook，并保留删除审计。外部文件站介质和 Registry 镜像本体不会被删除，只移除本 Release 的登记。
...
- 受影响的下游组件 Owner。
- 精确锁定父 Release 或传递受影响 Release 的场景 Owner。
