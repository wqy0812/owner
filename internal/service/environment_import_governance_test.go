...
func importGovernanceFixture(t *testing.T) (*Platform, domain.User, domain.Environment, EnvironmentExportDocument, domain.ComponentRelease) {
	...
	if err := testDatabase(p).CreateComponent(ctx, c); err != nil {
		t.Fatal(err)
	}
	r := domain.ComponentRelease{ID: "import-contract-r1", ComponentID: c.ID, LineID: "import-line", LineName: "Import", Version: "1.0.0", Status: domain.ReleaseDraft, Compatibility: domain.CompatibilityNotApplicable, RiskLevel: domain.RiskLow, CreatedAt: now, EnvironmentConstraints: map[string]any{}, Parameters: []domain.ParameterDefinition{
		{Name: "count", Description: "Instance count", Type: domain.ParameterTypeInteger, Enum: []any{2, 4}, Required: true, Modifiable: true, Visibility: domain.ParameterPublic, ValueProvider: domain.ParameterProviderEnvironmentOwner, EnvironmentBinding: &domain.EnvironmentParameterBinding{Kind: domain.EnvironmentBindingPrivate}},
	...
	if err := testDatabase(p).CreateComponentRelease(ctx, r); err != nil {
		t.Fatal(err)
	}
	doc, err := p.environments.ExportRevision(ctx, owner, environment.ID, environment.CurrentRevisionID, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, kind := range []string{"new", "existing"} {
		for _, name := range []string{"valid", "partial", "unknown_parameter", "wrong_type", "fractional_integer", "wrong_enum", "short_string", "unknown_variable", "sensitive_variable", "credential_collision", "nested_sensitive"} {
			t.Run(kind+"/"+name, func(t *testing.T) {
				...
				ctx := context.Background()
				before, _ := testDatabase(p).ListEnvironments(ctx)
				plan, err := p.environments.PreviewImport(ctx, owner, input)
				...
				if !valid {
					...
					if _, err := p.environments.Import(ctx, owner, input); !errors.Is(err, domain.ErrInvalid) {
						t.Fatalf("commit error=%v", err)
					}
					after, _ := testDatabase(p).ListEnvironments(ctx)
					current, _ := testDatabase(p).GetEnvironment(ctx, environment.ID, true)
				...
				imported, err := p.environments.Import(ctx, owner, input)
				if err != nil {
					t.Fatal(err)
				}
				exported, err := p.environments.ExportRevision(ctx, owner, imported.ID, imported.CurrentRevisionID, false)
				if err != nil {
					t.Fatal(err)
				}
		...
		for _, change := range []string{"removed_field", "removed_variable"} {
			t.Run(kind+"/"+change, func(t *testing.T) {
				...
				if change == "removed_variable" {
					if err := testDatabase(p).UpsertEnvironmentVariableDefinition(ctx, domain.EnvironmentVariableDefinition{ID: "import-variable", Name: "IMPORT_ZONE", Label: "Zone", CreatedBy: seed.PlatformAdminID, CreatedAt: time.Now().UTC()}); err != nil {
						t.Fatal(err)
					}
				...
				if change == "removed_field" {
					r.Parameters = nil
					if err := testDatabase(p).UpdateDraftRelease(ctx, r); err != nil {
						t.Fatal(err)
					}
				} else {
					if err := testDatabase(p).DeleteEnvironmentVariableDefinition(ctx, "import-variable", domain.AuditEvent{ID: "remove-import-variable", ActorID: seed.PlatformAdminID, Action: "variable.deleted", ResourceType: "variable", ResourceID: "import-variable", CreatedAt: time.Now().UTC()}); err != nil {
						t.Fatal(err)
					}
				}
				if _, err := p.environments.Import(ctx, owner, input); err == nil {
					t.Fatal("stale import accepted")
				}
				current, err := testDatabase(p).GetEnvironment(ctx, environment.ID, true)
