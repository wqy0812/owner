...
func restoreDefinitionTables(ctx context.Context, tx *sql.Tx, catalog Catalog, currentGeneration int64) error {
	...
	tables := catalogTableMap(catalog)
	for _, name := range []string{"components", "component_release_lines", "component_releases", "component_dependencies", "action_definitions", "component_playbook_files", "scenarios", "scenario_revisions", "component_release_artifacts", "component_release_images"} {
