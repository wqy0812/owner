...
func (s *Store) CreateComponentImageBuild(ctx context.Context, build domain.ComponentImageBuild) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO component_image_builds(id,release_id,requested_by,status,dockerfile_sha256,image_tag,image_ref,image_digest,error_text,created_at,started_at,finished_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,
		build.ID, build.ReleaseID, build.RequestedBy, build.Status, build.DockerfileSHA256, build.ImageTag, build.ImageRef, build.ImageDigest, build.Error, timeText(build.CreatedAt), ptrTimeText(build.StartedAt), ptrTimeText(build.FinishedAt))
...
func (s *Store) GetComponentImageBuild(ctx context.Context, id string) (domain.ComponentImageBuild, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id,release_id,requested_by,status,dockerfile_sha256,image_tag,image_ref,image_digest,error_text,created_at,started_at,finished_at FROM component_image_builds WHERE id=?`, id).Scan(
		&build.ID, &build.ReleaseID, &build.RequestedBy, &build.Status, &build.DockerfileSHA256, &build.ImageTag, &build.ImageRef, &build.ImageDigest, &build.Error, &created, &started, &finished)
	if err != nil {
...
func (s *Store) ListComponentImageBuilds(ctx context.Context, releaseID string, limit int) ([]domain.ComponentImageBuild, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id,release_id,requested_by,status,dockerfile_sha256,image_tag,image_ref,image_digest,error_text,created_at,started_at,finished_at FROM component_image_builds WHERE release_id=? ORDER BY created_at DESC LIMIT ?`, releaseID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var created string
		var started, finished sql.NullString
		if err := rows.Scan(&build.ID, &build.ReleaseID, &build.RequestedBy, &build.Status, &build.DockerfileSHA256, &build.ImageTag, &build.ImageRef, &build.ImageDigest, &build.Error, &created, &started, &finished); err != nil {
			return nil, err
		}
		build.CreatedAt = parseTime(created)
		build.StartedAt = parseNullTime(started)
