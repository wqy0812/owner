...
func insertEnvironmentRevision(ctx context.Context, tx *sql.Tx, r domain.EnvironmentRevision) error {
	...
	_, err := tx.ExecContext(ctx, `INSERT INTO environment_revisions(id,environment_id,revision,facts_json,inventory_json,parameters_json,credential_refs_json,max_concurrent,created_at) VALUES(?,?,?,?,?,?,?,?,?)`, r.ID, r.EnvironmentID, r.Revision, jsonText(r.Facts), inventory, jsonText(r.Parameters), jsonText(r.CredentialRefs), r.MaxConcurrent, timeText(r.CreatedAt))
	return mapSQLError(err)
}
...
func scanEnvironmentRevision(row scanner) (domain.EnvironmentRevision, error) {
	...
	var facts, inventory, parameters, refs, created string
	err := row.Scan(&r.ID, &r.EnvironmentID, &r.Revision, &facts, &inventory, &parameters, &refs, &r.MaxConcurrent, &created)
...
func (s *Store) GetEnvironmentRevision(ctx context.Context, id string) (domain.EnvironmentRevision, error) {
	r, err := scanEnvironmentRevision(s.db.QueryRowContext(ctx, `SELECT id,environment_id,revision,facts_json,inventory_json,parameters_json,credential_refs_json,max_concurrent,created_at FROM environment_revisions WHERE id=?`, id))
...
func (s *Store) ListEnvironmentRevisions(ctx context.Context, environmentID string) ([]domain.EnvironmentRevision, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id,environment_id,revision,facts_json,inventory_json,parameters_json,credential_refs_json,max_concurrent,created_at FROM environment_revisions WHERE environment_id=? ORDER BY revision DESC`, environmentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
