...
func (s *Store) IsActionReceiptRecovered(ctx context.Context, receipt domain.ActionExecutionReceipt) (bool, error) {
	tx, err := s.db.BeginTx(ctx, &sql.TxOptions{ReadOnly: true})
	if err != nil {
		return false, err
	}
	defer tx.Rollback()
	var latest domain.ActionExecutionReceipt
	...
	rows, err := tx.QueryContext(ctx, runSelect+` WHERE environment_id=? AND kind='scenario_run' AND status='succeeded' AND json_extract(execution_snapshot_json,`+runSnapshotContractPath+`)='clusterforge-run-v1' AND json_extract(execution_snapshot_json,`+runSnapshotScenarioModePath+`)='baseline_verify' AND finished_at IS NOT NULL ORDER BY finished_at DESC`, receipt.EnvironmentID)
	if err != nil {
		return false, err
	}
	runs := []domain.Run{}
	...
	for _, run := range runs {
		...
		if baseline.InstallationDigest != actualDigest || run.Snapshot.ScenarioExecution.Baseline.InstallationDigest != actualDigest {
			continue
		}
		baselineRunID := run.Snapshot.ScenarioExecution.Baseline.RunID
		if baselineRunID == "" || baseline.RunID != baselineRunID {
			continue
		}
		testOnly := run.Snapshot.ScenarioExecution.Baseline.TestOnly
		if baseline.TestOnly != testOnly || (baseline.State == "test") != testOnly {
			continue
		}
		recovered := run.Snapshot.ScenarioExecution.RecoveredReceipts
