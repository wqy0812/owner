...
func createSuccessfulScenarioTestEvidence(t *testing.T, s *Store, revision *domain.ScenarioRevision, releases ...domain.ComponentRelease) string {
	...
	environmentRevision := domain.EnvironmentRevision{
		...
		Facts: map[string]any{"architecture": "amd64"}, Inventory: inventory, Variables: map[string]string{}, CredentialRefs: []domain.CredentialRef{}, CreatedAt: testNow,
	}
	...
	run := domain.Run{
		...
		Snapshot: runfixture.Snapshot(map[string]any{
			"scenarioRevisionSpecDigest": domain.ScenarioRevisionSpecDigest(*revision),
			"steps":                      steps, "scenarioContractVersion": 2, "executionMode": "install", "targetNodes": []domain.ScenarioTargetNode{}, "acceptanceJobIds": []string{"business"},
		}),
	}
...
func TestListRunsForComponentReleaseUsesImmutableRunEvidence(t *testing.T) {
	...
	environmentRevision := domain.EnvironmentRevision{ID: "evidence-lab-r1", EnvironmentID: environment.ID, Revision: 1, Facts: map[string]any{"architecture": "amd64"}, Inventory: inventory, Variables: map[string]string{}, CredentialRefs: []domain.CredentialRef{}, CreatedAt: testNow}
	if err := s.CreateEnvironment(ctx, environment, environmentRevision); err != nil {
		t.Fatal(err)
	}
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "evidence-scenario-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionDraft, Graph: domain.ScenarioGraph{Nodes: []domain.ScenarioNode{}, Edges: []domain.ScenarioEdge{}}, CreatedAt: testNow}
	...
	base := domain.Run{RequestedBy: "component-owner-a", EnvironmentID: environment.ID, EnvironmentRevisionID: environmentRevision.ID, Snapshot: runfixture.Snapshot(map[string]any{}), CreatedAt: testNow}
	componentRun := base
	...
	scenarioTest.Snapshot = runfixture.Snapshot(map[string]any{"steps": []any{map[string]any{"releaseId": release.ID}, map[string]any{"releaseId": release.ID}, map[string]any{"releaseId": otherRelease.ID}}})
	create(scenarioTest)
	scenarioRun := base
	...
	scenarioRun.Snapshot = runfixture.Snapshot(map[string]any{"steps": []any{map[string]any{"releaseId": release.ID}}})
	create(scenarioRun)
	unrelated := base
	...
	rollback.Snapshot = runfixture.Snapshot(map[string]any{"steps": []any{map[string]any{"releaseId": release.ID}}})
	create(rollback)
...
func TestCandidateReleaseVisibilityAndAtomicScenarioPublish(t *testing.T) {
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "candidate-scene-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionTestPassed, Graph: graph, CreatedAt: testNow, TestPassedAt: ptr(testNow)}
	if err := s.CreateScenario(ctx, scenario, revision); err != nil {
		t.Fatal(err)
	}
...
func TestCandidateReleaseSetPublishRollsBackOnStaleMember(t *testing.T) {
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "stale-set-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionTestPassed, Graph: domain.ScenarioGraph{Nodes: []domain.ScenarioNode{
...
func TestScenarioTestEvidenceRejectsChangedReleaseDefinition(t *testing.T) {
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}},
...
func TestEnvironmentRollbackFenceIsEnforcedByDatabase(t *testing.T) {
	...
	newEnvironment := func(t *testing.T, s *Store, id string) domain.EnvironmentRevision {
		...
		revision := domain.EnvironmentRevision{ID: id + "-r1", EnvironmentID: id, Revision: 1, Facts: map[string]any{"architecture": "amd64"}, Inventory: inventory, Variables: map[string]string{}, CredentialRefs: []domain.CredentialRef{}, CreatedAt: testNow}
	...
	active := func(id string, kind domain.RunKind, revision domain.EnvironmentRevision) domain.Run {
		return domain.Run{ID: id, Kind: kind, Status: domain.RunAwaitingApproval, RequestedBy: "environment-owner-a", EnvironmentID: revision.EnvironmentID, EnvironmentRevisionID: revision.ID, Snapshot: domain.SnapshotFromExecutionPlan(domain.RunExecutionPlan{Steps: []domain.RunPlanStep{{ID: "step", NodeID: "node"}}}), CreatedAt: testNow}
	}
	...
	t.Run("active run blocks rollback", func(t *testing.T) {
		...
		if err := s.CreateRun(ctx, active("other-first", domain.RunComponentTest, revision), nil); err != nil {
			t.Fatal(err)
		}
...
func TestBatchApprovalIsAtomicAndPreservesFIFOOrder(t *testing.T) {
	...
	revision := domain.EnvironmentRevision{ID: "batch-env-r1", EnvironmentID: environment.ID, Revision: 1, Facts: map[string]any{"architecture": "amd64"}, Inventory: inventory, Variables: map[string]string{}, CredentialRefs: []domain.CredentialRef{}, CreatedAt: testNow}
	if err := s.CreateEnvironment(ctx, environment, revision); err != nil {
		t.Fatal(err)
	}
	for index := 1; index <= 2; index++ {
		run := domain.Run{ID: fmt.Sprintf("batch-run-%d", index), Kind: domain.RunComponentTest, Status: domain.RunAwaitingApproval, RequestedBy: "scenario-owner-a", EnvironmentID: environment.ID, EnvironmentRevisionID: revision.ID, Destructive: true, Snapshot: domain.SnapshotFromExecutionPlan(domain.RunExecutionPlan{Steps: []domain.RunPlanStep{{ID: "step", NodeID: "node"}}}), CreatedAt: testNow.Add(time.Duration(index) * time.Second)}
	...
	stale := domain.Run{ID: "batch-stale-run", Kind: domain.RunComponentTest, Status: domain.RunAwaitingApproval, RequestedBy: "scenario-owner-a", EnvironmentID: environment.ID, EnvironmentRevisionID: revision.ID, Destructive: true, Snapshot: domain.SnapshotFromExecutionPlan(domain.RunExecutionPlan{Steps: []domain.RunPlanStep{{ID: "step", NodeID: "node"}}}), CreatedAt: testNow.Add(3 * time.Minute)}
...
func TestScenarioEnvironmentRunApprovalAndFIFO(t *testing.T) {
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "cluster-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionDraft, Graph: graph, CreatedAt: testNow}
	if err := s.CreateScenario(ctx, scenario, revision); err != nil {
		t.Fatal(err)
	}
	...
	run1 := domain.Run{ID: "run-1", Kind: domain.RunComponentTest, Status: domain.RunAwaitingApproval, RequestedBy: "scenario-owner-a", EnvironmentID: env.ID, EnvironmentRevisionID: envRev.ID, ComponentReleaseID: "runtime-1", Destructive: true, Snapshot: runfixture.Snapshot(map[string]any{"steps": []any{map[string]any{"id": "step", "nodeId": "node", "releaseId": "runtime-1"}}}), CreatedAt: testNow}
	...
	componentTest := domain.Run{
		...
		ComponentReleaseID: "runtime-1", Snapshot: domain.SnapshotFromExecutionPlan(domain.RunExecutionPlan{Steps: []domain.RunPlanStep{{ID: "step", NodeID: "node", ReleaseID: "runtime-1"}}}), CreatedAt: testNow.Add(20 * time.Minute),
	}
...
func TestFailInvalidActiveRunsKeepsCorruptEntriesOutOfQueue(t *testing.T) {
	...
	revision := domain.EnvironmentRevision{ID: "reconcile-env-r1", EnvironmentID: environment.ID, Revision: 1, Facts: map[string]any{"architecture": "amd64"}, Inventory: json.RawMessage(`{"hosts":[]}`), CreatedAt: testNow}
	if err := s.CreateEnvironment(ctx, environment, revision); err != nil {
		t.Fatal(err)
	}
	plan := map[string]any{"steps": []any{map[string]any{"id": "step", "nodeId": "node", "releaseId": "reconcile-release"}}}
	invalid := domain.Run{ID: "invalid-awaiting", Kind: domain.RunComponentTest, Status: domain.RunAwaitingApproval, RequestedBy: "component-owner-a", EnvironmentID: environment.ID, EnvironmentRevisionID: revision.ID, ComponentReleaseID: "reconcile-release", Destructive: true, Snapshot: runfixture.Snapshot(plan), CreatedAt: testNow}
...
func TestRestartInterruptsRunAndReleasesScenarioTestingState(t *testing.T) {
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "restart-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionTesting, Graph: domain.ScenarioGraph{}, CreatedAt: testNow}
	if err := s.CreateScenario(ctx, scenario, revision); err != nil {
		t.Fatal(err)
	}
	...
	environmentRevision := domain.EnvironmentRevision{ID: "restart-env-r1", EnvironmentID: environment.ID, Revision: 1, Facts: map[string]any{"architecture": "amd64"}, Inventory: json.RawMessage(`{"hosts":[]}`), CreatedAt: testNow}
	if err := s.CreateEnvironment(ctx, environment, environmentRevision); err != nil {
		t.Fatal(err)
	}
	run := domain.Run{ID: "restart-run", Kind: domain.RunScenarioTest, Status: domain.RunRunning, RequestedBy: "scenario-owner-a", EnvironmentID: environment.ID, EnvironmentRevisionID: environmentRevision.ID, ScenarioRevisionID: revision.ID, Snapshot: runfixture.Snapshot(map[string]any{"steps": []any{map[string]any{"id": "step", "releaseId": release.ID}}}), CreatedAt: testNow}
	if err := insertStoredRunForTest(ctx, s, run); err != nil {
		t.Fatal(err)
	}
...
func TestScenarioGraphEditInvalidatesTestAndReleasedIsImmutable(t *testing.T) {
	ctx := context.Background()
	s := newTestStore(t)
	component := componentFixture("editable-component", "component-owner-a")
	if err := s.CreateComponent(ctx, component); err != nil {
		t.Fatal(err)
	}
	if err := s.CreateComponentRelease(ctx, releaseFixture("release", component.ID, "1.0", domain.ReleaseReleased)); err != nil {
		t.Fatal(err)
	}
	...
	revision := domain.ScenarioRevision{EnvironmentConstraints: map[string]any{"architecture": []string{"amd64"}}, ID: "editable-r1", ScenarioID: scenario.ID, Revision: 1, Status: domain.RevisionDraft, Graph: graph, CreatedAt: testNow}
	if err := s.CreateScenario(ctx, scenario, revision); err != nil {
		t.Fatal(err)
	}
	if err := s.SetScenarioRevisionStatus(ctx, revision.ID, []domain.RevisionStatus{domain.RevisionDraft}, domain.RevisionTestPassed, testNow); err != nil {
...
func TestResetClearsPendingActionFileMutations(t *testing.T) {
	s := newTestStore(t)
	ctx := context.Background()
	if err := s.CreateComponent(ctx, componentFixture("reset-component", "component-owner-a")); err != nil {
		t.Fatal(err)
	}
	if err := s.CreateComponentRelease(ctx, releaseFixture("reset-release", "reset-component", "1.0.0", domain.ReleaseDraft)); err != nil {
		t.Fatal(err)
	}
	if _, err := s.db.ExecContext(ctx, `INSERT INTO playbook_action_mutations(id,release_id,workspace_root,relative_path,before_exists,before_content,created_at) VALUES('pending','reset-release','managed/reset','install.yml',0,X'',?)`, timeText(testNow)); err != nil {
		t.Fatal(err)
	}
	if err := s.Reset(ctx); err != nil {
		t.Fatalf("reset blocked by pending Action journal: %v", err)
	}
	var count int
...
func insertStoredRunForTest(ctx context.Context, s *Store, r domain.Run) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO runs(id,kind,status,requested_by,environment_id,environment_revision_id,component_release_id,scenario_revision_id,action_kind,destructive,execution_snapshot_json,artifact_digest,created_at,started_at,finished_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, r.ID, r.Kind, r.Status, r.RequestedBy, r.EnvironmentID, r.EnvironmentRevisionID, nullString(r.ComponentReleaseID), nullString(r.ScenarioRevisionID), r.Action, r.Destructive, jsonText(r.Snapshot), r.ArtifactDigest, timeText(r.CreatedAt), ptrTimeText(r.StartedAt), ptrTimeText(r.FinishedAt))
	return err
}
